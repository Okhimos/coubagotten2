		...
		if (!IsValid(player.cwHoldingEnt)) then
								...
								if (ragdollPlayer) and !ragdollPlayer:GetNetVar("IsDragged") and !ragdollPlayer.BeingPickedUp then
									if ragdollPlayer:Alive() then
										if ragdollPlayer.possessor then
											Schema:EasyText(player, "chocolate", "Как бы вы ни старались, вы не сможете удержать этого человека.!");
											return;
										end
									
										if ragdollPlayer.stabilityStunned and !player:HasBelief("wrestle_subdue") then
											Schema:EasyText(player, "chocolate", "Вы не сможете поднять человека, пока он находится в сознании, если у вас нет убеждения «Схватить и Удержать»!");
											return;
										end
									end
									...
									--if (Clockwork.player:GetUnragdollTime(ragdollPlayer) != 0) or !ragdollPlayer:Alive() then
										...
										Clockwork.chatBox:AddInTargetRadius(player, "me", "начинает поднимать тело перед собой.", player:GetPos(), config.Get("talk_radius"):Get() * 2);
			...
			elseif (key == IN_ATTACK) then
				if (Clockwork.player:GetAction(player) == "pickupragdoll") then
					...
					Clockwork.chatBox:AddInTargetRadius(player, "me", "отпускает тело, что держал в руках.", player:GetPos(), config.Get("talk_radius"):Get() * 2);
...
function cwPickupObjects:CanHandsPickupEntity(player, entity, trace)
	if (!player:IsAdmin() and player.cwObserverMode) then
		Schema:EasyText(player, "grey", "Ты не можешь делать это, находясь в Наблюдателях!!");
		return false;
	end
	
	if cwDueling and cwDueling:PlayerIsInMatchmaking(player) then
		Schema:EasyText(player, "grey", "Ты не можешь делать это, находясь в Дуэли!");
		return false;
	end

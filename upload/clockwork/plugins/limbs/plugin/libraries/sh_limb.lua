...
if (SERVER) then
	...
	function Clockwork.limb:BuildTooltip(hitGroup, x, y, width, height, frame)
		...
		if bleeding then
			frame:AddText("Кровотечение", Color(200, 40, 40), "nov_IntroTextSmallDETrooper", 1);
		end
		
		if infected then
			if infected == 1 then
				frame:AddText("Малая Инфекция", Color(200, 40, 40), "nov_IntroTextSmallDETrooper", 1);
			elseif infected == 2 then
				frame:AddText("Инфекция", Color(200, 40, 40), "nov_IntroTextSmallDETrooper", 1);
			elseif infected == 3 then
				frame:AddText("Обширная Инфекция", Color(200, 40, 40), "nov_IntroTextSmallDETrooper", 1);
			end
		end

...
function cwStorage:HUDPaintEntityTargetID(entity, info)
	...
	if (Clockwork.entity:IsPhysicsEntity(entity)) then
		...
		if (self.containerList[model]) then
			...
			info.y = Clockwork.kernel:DrawInfo("Ты можешь положить туда свои вещи", info.x, info.y, colorWhite, info.alpha)
...
function cwStorage:HUDPaint()
	if (LockpickInfo and LockpickInfo.DoingLockpick) then
		...
		if (LockpickInfo.PressedEnterAlpha > 0) then
			draw.SimpleText("Нажми 'Enter', чтобы начать взлом.", "cwMainText", x, tipY, Color(255, 0, 0, LockpickInfo.PressedEnterAlpha));
		end;
		
		draw.SimpleText("Нажми 'R', чтобы выйти.", "cwMainText", x, LockpickInfo.ExitPositionY, Color(255, 0, 0));
		...
		if (LockpickInfo.RequiredPicks and LockpickInfo.SuccessfulPicks) then
			...
			local progressText = "ПРОГРЕСС: "..LockpickInfo.SuccessfulPicks.."/"..LockpickInfo.RequiredPicks;

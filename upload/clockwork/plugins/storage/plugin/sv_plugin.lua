	...
	if (IsValid(entity) and Clockwork.entity:IsPhysicsEntity(entity)) then
		...
		if (cwStorage.containerList[model]) then
			...
			if (entity.cwPassword == password) then
				...
				Schema:EasyText(player, "peru", "Вы ввели недействительный пароль!")
			end
		end
	end
end)
	...
	if (!entity.LockpickCooldown or CurTime() > entity.LockpickCooldown) then
		if (!player.LockpickCooldown or CurTime() > player.LockpickCooldown) then
			...
			if (lockstrength == 4) then
				Schema:EasyText(player, "peru", "Ты не можешь вскрыть этот ящик!");
				return;
			end;
			...
			if cwBeliefs and player.HasBelief then
				if (lockstrength == 3) and not player:HasBelief("safecracker") then
					Schema:EasyText(player, "chocolate", "Ты недостаточно опытен для открытия этого замка!");
					return;
			...
			Schema:EasyText(player, "peru", "Ты не можешь взламывать еще "..math.ceil(player.LockpickCooldown - CurTime()).." секунд!");
		end;
	else
		Schema:EasyText(player, "peru", "Ты не можешь взламывать еще "..math.ceil(entity.LockpickCooldown - CurTime()).." секунд!");
	end;
end;
	...
	if (player:HasItemByID("lockpick")) then
		...
		if (timeout == true) then
			if entity.cwLockTier >= 3 and (math.random(1, 10) == 1) then
				message = "Одно неверноее движение и твоя отмычка превратилась в кусок мусора!";
				bDidBreak = true;
			else
				...
				if (quality <= 10) then
					message = "Отмычка с хрустом сломалась!";
					bDidBreak = true;
				else
...
function cwStorage:FinishLockpick(player, entity)
	...
	if (cwStorage.containerList[model]) then
		...
		Schema:EasyText(player, "olivedrab", "Ты вскрыл замок.");
	...
	if (IsValid(entity) and Clockwork.entity:IsPhysicsEntity(entity)) then
		...
		if (cwStorage.containerList[model]) then
			...
			if (entity.cwPassword == password) then
				...
				Schema:EasyText(player, "peru", "Ты ввел недействительный пароль!");
			end;
		end;
	end;
end);
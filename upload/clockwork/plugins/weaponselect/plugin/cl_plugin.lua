		...
		if #ammo > 0 then
			if itemTable.ammoCapacity > 1 then
				if itemTable.usesMagazine then
					...
					if clipItem then
						ammoText = "Заряженный Патрон: "..ammo[1].." ("..tostring(#ammo).."/"..tostring(clipItem.ammoMagazineSize or weapon.Primary.ClipSize)..")";
					else
						ammoText = "Заряженный Патрон: "..ammo[1].." ("..tostring(#ammo).."/"..tostring(weapon.Primary.ClipSize)..")";
					end
				else
					-- Likely is multi-barreled gun.
					ammoText = "Заряженный Патронt: ";
				...
				ammoText = "Заряженный Патрон: "..ammo[1];
			end
		end
	end

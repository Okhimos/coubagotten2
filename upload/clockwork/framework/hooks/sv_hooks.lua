				...
				if itemTable.repairItem then
					...
					if repairItemTable then
						...
						if repairItemTable:GetCondition() <= 0 then
							...
							Schema:EasyText(player, "olivedrab", "Ты починил "..itemTable.name.." до "..tostring(math.Round(itemTable:GetCondition(), 2))..", используя последнюю часть набора в процессе!");
						else
							Schema:EasyText(player, "green", "Ты починил "..itemTable.name.." до "..tostring(math.Round(itemTable:GetCondition(), 2))..".");
							Clockwork.inventory:Rebuild(player);
						end
					else
						Schema:EasyText(player, "chocolate", "Ты не можешь это починить!");
						return false;
					end
				end
			else
				Schema:EasyText(player, "peru", "Тебе не нужно чинить этот предмет.");
				return false;
			end
		end
	elseif (class == "cw_item" and arguments == "cwItemExamine") then
		...
		if itemEngraving and itemEngraving ~= "" then
			...
			if itemKills and itemKills > 0 then
				examineText = examineText.." Теперь этот предмет имеет гравировку \'"..itemEngraving.."\' а также количество убийств "..tostring(itemKills).." kills.";
			else
				examineText = examineText.." Теперь этот предмет имеет \'"..itemEngraving.."\' в виде гравировки.";
			end
		end

		if table.HasValue(conditionTextCategories, itemTable.category) then
			if itemCondition >= 90 then
				examineText = examineText.." Этот предмет находится в прекрасном состоянии.";
			elseif itemCondition < 90 and itemCondition >= 60 then
				examineText = examineText.." Этот предмет находится в слегка потрепанном состоянии.";
			elseif itemCondition < 60 and itemCondition >= 30 then
				examineText = examineText.." Этот предмет находится в плохом состоянии.";
			elseif itemCondition < 30 and itemCondition > 0 then
				examineText = examineText.." Этот предмет вот-вот сломается.";
			elseif itemCondition <= 0 then
				if itemTable:IsBroken() then
					examineText = examineText.." Окончательно сломан и представляет из себя лишь мусор.";
				else
					examineText = examineText.." Сломан, но еще может быть использован.";
				end
			end
		elseif itemTable.category == "Shot" and itemTable.ammoMagazineSize then
			local rounds = itemTable:GetAmmoMagazine();
			
			examineText = examineText.." Магазин заряжен "..tostring(rounds).." "..itemTable.ammoName.." патрон.";
		end
	...
	if (IsValid(entity)) then
		...
		if (scalePropCost > 0) then
			...
			if (Clockwork.player:CanAfford(player, info.cost)) then
				...
				Schema:EasyText(player, "chocolate", "Тебе нужно иметь еще "..Clockwork.kernel:FormatCash(info.cost - player:GetCash(), nil, true).." чтобы купить это!")
				entity:Remove()
				return
			end
		end
...
function GM:ChatBoxMessageAdded(info)
	if (info.class == "ic") then
		Clockwork.kernel:PrintLog(LOGTYPE_GENERIC, info.speaker:Name().." говорит \""..info.text.."\"")
	--[[elseif (info.class == "yell") then
		Clockwork.kernel:PrintLog(LOGTYPE_GENERIC, info.speaker:Name().." кричит \""..info.text.."\"")
...
function GM:PlayerCancelGetUp(player)
	if (player:Alive()) then
		...
		if (!player.NextCancelMessage or player.NextCancelMessage < curTime) then
			...
			local performs = {
				["restrained"] = {
					"подавлен и прекращает #HIS попытки встать.",
					"оказывается зажат и перестает двигаться.",
					"не может двигаться.",
					"удерживается и перестает двигаться.",
					"#HE пытается двигаться, но не может.",
				},
				["unrestrained"] = {
					"перестает двигаться.",
					"лежит смирно.",
					"валится обратно на землю.",
					"прекращает #HIS попытки подняться на ноги.",
				},
			};
			
			local gender = player:GetGender();
			local he, his = "Он", "свои";
	...
	if (player:Alive()) then
		...
		if (!player.NextGetUpMessage or player.NextGetUpMessage < curTime) then
			...
			local performs = {
				["restrained"] = {
					"подавлен и прекращает #HIS попытки встать.",
					"связан, из-за чего не может встать.",
				},
				["unrestrained"] = {
					"начинает вставать на ноги.",
					"начинает подниматься на #HIS ноги.",
					"начинает вставать.",
			...
			local gender = player:GetGender();
			local he, his, himself = "свои", "свои", "свои";
				if (gender == GENDER_FEMALE) then
					he = "свои"; his = "свои"; himself = "свои";
				end;
			local restrained = (player:GetNetVar("IsDragged") and "restrained") or "unrestrained";

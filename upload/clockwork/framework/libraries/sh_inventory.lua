		...
		if (itemTable) then
			...
			if (itemAction == "repair") then
				...
				if itemCondition <= 0 then
					if !cwBeliefs or !player:HasBelief("artisan") then
						Clockwork.player:Notify(player, "Вы не можете починить сломанный предмет!");
						return false;
					end
				end
						...
						if (repairItemTable) then
							if repairItemTable then
								...
								if repairItemTable:GetCondition() <= 0 then
									...
									Schema:EasyText(player, "olivedrab", "Вы починили ваш "..itemTable.name.." до "..tostring(math.Round(itemTable:GetCondition(), 2))..", используя остатки ремонтного набора.");
								else
									player:EmitSound("generic_ui/randomize_0"..math.random(1, 2)..".wav")
									Schema:EasyText(player, "green", "Вы почини ваш "..itemTable.name.." до "..tostring(math.Round(itemTable:GetCondition(), 2))..".");
									Clockwork.inventory:Rebuild(player);
								end
							else
								Schema:EasyText(player, "chocolate", "У вас нет предмета, при помощи которого вы смогли бы починить это!");
								return false;
							end
						end
					end
				else
					Clockwork.player:Notify(player, "Этот предмет уже в идеальном состоянии.");
					return false;
				end
							...
							if !cwBeliefs or (cwBeliefs and player:HasBelief("smith")) then
								...
								for i = 1, #cwRecipes.smithyLocations do
									if player:GetPos():DistToSqr(cwRecipes.smithyLocations[i]) < (256 * 256) then
										...
										Clockwork.player:Notify(player, "Вы расплавили ваш "..itemTable.name.." на компоненты.");
								...
								if not smithy_found then
									Clockwork.player:Notify(player, "Вам нужно стоять рядом с кузней, чтобы расплавить этот предмет!");
									return false;
								end
							else
								Clockwork.player:Notify(player, "Вам нужно верование 'Подмастерье' чтобы расплавить этот предмет!");
								return false;
							end
						else
							Clockwork.player:Notify(player, "Вам нужен уголь чтобы расплавить этот предмет!");
						end
					elseif itemTable.components.breakdownType == "breakdown" then
						...
						if breakdownItemTable then
							...
							--if conditionTaken <= itemCondition then
								...
								Clockwork.player:Notify(player, "Вы разобрали ваш "..itemTable.name.." на компоненты.");
							...
							Clockwork.player:Notify(player, "У вас нет предмета, с помощью которого вы могли бы это разобрать!");
							return false;
						end
					end
				end
			...
			elseif (itemAction == "drop") then
				...
				if (player:GetShootPos():Distance(position) <= 192) then
					...
					Clockwork.player:Notify(player, "Вы не можете выбросить предмет так далеко!");
				end;
			elseif (itemAction == "use") then
				if (player:InVehicle() and itemTable.useInVehicle == false) then
					Clockwork.player:Notify(player, "Вы не можете использовать этот предмет в транспорте!");
			...
			elseif (itemAction == "examine") then
				...
				if itemEngraving and itemEngraving ~= "" then
					...
					if itemKills and itemKills > 0 then
						examineText = examineText.." На нем выгравированно \'"..itemEngraving.."\', вместе с пометкой "..tostring(itemKills).." убийств.";
					else
						examineText = examineText.." На нем выгравированно \'"..itemEngraving.."\'.";
					end
				end
				
				if table.HasValue(conditionTextCategories, itemTable.category) then
					if itemCondition >= 90 then
						examineText = examineText.." Оно в идеальном состоянии.";
					elseif itemCondition < 90 and itemCondition >= 60 then
						examineText = examineText.." Оно в немного поврежденом состоянии.";
					elseif itemCondition < 60 and itemCondition >= 30 then
						examineText = examineText.." Оно в очень плохом состоянии.";
					elseif itemCondition < 30 and itemCondition > 0 then
						examineText = examineText.." Оно на грани поломки.";
					elseif itemCondition <= 0 then
						if itemTable:IsBroken() then
							examineText = examineText.." Оно полностью сломано.";
						else
							examineText = examineText.." Оно полностью сломано, но вы можете это к чему-то приспособить.";
						end
					end
				elseif itemTable.category == "Shot" and itemTable.ammoMagazineSize then
					local rounds = itemTable:GetAmmoMagazine();
					
					examineText = examineText.." В магазине "..tostring(rounds).." "..itemTable.ammoName.." пуль.";
				end
			...
			Clockwork.player:Notify(player, "Вы не владеете этим предметом!");
		end;
	end

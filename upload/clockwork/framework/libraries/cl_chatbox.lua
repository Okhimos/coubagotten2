	...
	if (info.shouldHear) then
		...
		if info.font then
			if info.font == "Voltism" then
				...
				if lastChar == "?" then
					sayText = "трещит как тостер";
				elseif lastChar == "!" then
					sayText = "воет как сирена";
				end
			end
...
Clockwork.chatBox:RegisterDefaultClass("yell", "ic", function(info)
	...
	if info.font then
		if info.font == "Voltism" then
			Clockwork.chatBox:Add(info.filtered, nil, color, info.name.." верещит \""..info.text.."\"", info.font, "noTime");
		else
...
Clockwork.chatBox:RegisterDefaultClass("radio_eavesdrop", "ic", function(info)
	if (info.shouldHear) then
		...
		Clockwork.chatBox:Add(info.filtered, nil, color, info.name.." передает по рации \""..info.text.."\"", info.font);
	end;
end);

Clockwork.chatBox:RegisterDefaultClass("goreevent", "ic", function(info)
	Clockwork.chatBox:Add(info.filtered, nil, Color(200, 100, 50, 255), "(ГОРЕЙСКИЙ ЛЕС) "..info.text);
end);
...
Clockwork.chatBox:RegisterDefaultClass("towerevent", "ic", function(info)
	Clockwork.chatBox:Add(info.filtered, nil, Color(200, 100, 50, 255), "(БАШНЯ) "..info.text);
end);

Clockwork.chatBox:RegisterDefaultClass("wastelandevent", "ic", function(info)
	Clockwork.chatBox:Add(info.filtered, nil, Color(200, 100, 50, 255), "(ПУСТОШИ) "..info.text);
end);
...
Clockwork.chatBox:RegisterDefaultClass("localevent", "ic", function(info)
	Clockwork.chatBox:Add(info.filtered, nil, Color(200, 100, 50, 255), "(РЯДОМ) "..info.text);
end);

	...
	if (!hook.Run("PreMainMenuRebuild", self)) then
		...
		function self.closeMenu:Paint()
			if (self:GetHovered()) then
				local texts = {"ЗАКРЫТЬ", "зАкРыТь", "zАкРыТь"};
		...
		local width, height = Clockwork.kernel:GetCachedTextSize(smallTextFont, "Жертвы");
		...
		-- Rituals Button
		local width, height = Clockwork.kernel:GetCachedTextSize(smallTextFont, "Ритуалы");
		...
		-- Crafting Button
		local width, height = Clockwork.kernel:GetCachedTextSize(smallTextFont, "Крафт");
		...
		-- Beliefs Button
		local width, height = Clockwork.kernel:GetCachedTextSize(smallTextFont, "Убеждения");
		
		self.beliefsmenu = vgui.Create("cwLabelButton", self);
		self.beliefsmenu:SetFont(Clockwork.option:GetFont("menu_text_tiny"));
		self.beliefsmenu:SetText("Убеждения");
		...
		function self.beliefsmenu:Paint()
			...
			if (self:GetHovered()) then
				local texts = {"УБЕЖДЕНИЯ", "УбЕжДеНиЯ", "уБеЖдЕнИя"};
		...
		if (Clockwork.Client:IsAdmin()) then
			...
			self.adminlabel:SetText(string.upper("[АДМИН]"));
		...
		for k, v in pairs(Clockwork.menuitems.stored) do
			...
			if (button) then
				...
				elseif (string.find(string.upper(v.text), "SETTINGS")) then
					local width, height = Clockwork.kernel:GetCachedTextSize(smallTextFont, "Настройки");
					button:SetPos(ScrW() - width - 40, height);
					button:SetSize(width, height);
					button:SetText("Настройки");
					buttons[#buttons + 1] = button;
				elseif (string.find(string.upper(v.text), "MANIFESTO")) then
					local width, height = Clockwork.kernel:GetCachedTextSize(smallTextFont, "Манифест");
		...
		self.statusInfo.statusFrame.statusText:SetText("Эффекты");

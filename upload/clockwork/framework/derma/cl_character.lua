	...
	if (!hook.Run("PreCharacterMenuInit", self)) then
		...
		self.createButton:SetCallback(function(panel)
			if (table.Count(Clockwork.character:GetAll()) >= Clockwork.player:GetMaximumCharacters()) then
				return Clockwork.character:SetFault("Ты не можешь создать больше персонажей! Попробуй очистить Некропль!");
			end;
		...
		self.disconnectButton:SetText("СБЕЖАТЬ");
		...
		self.disconnectButton:SetCallback(function(panel)
			if (Clockwork.Client:HasInitialized()) then
				...
				Derma_Query("Серьезно хочешь сбежать?", "Disconnect", "Да", function()
					Derma_Query("Уверен в своем решении?", "Disconnect", "Нет...Я останусь подольше.", function() end, "Да, выпустите меня.", function()
						Derma_Query("Ты уверен, что желаешь ЭТОГО? Тебя ЛЮБЯТ!", "Disconnect", "Я СКАЗАЛ ВЫПУСТИТЕ!", function()
							print("ладно..Пока! Пока-а-а!");
							RunConsoleCommand("disconnect");
						end, "Ладно..Останусь. Пожалуй.", function() end);
					end);
				end, "No", function() end);
			end;
		...
		function self.disconnectButton:Paint(w, h)
			if (self:GetHovered()) then
				local texts = {"УбЕжАтЬ в СтРаХе", "СбЕжАтЬ", "БеГи!"};
	...
	if (!hook.Run("PreCharacterMenuThink", self)) then
		...
		if (IsValid(self.disconnectButton)) then
			...
			if (Clockwork.Client:HasInitialized()) then
				...
				self.disconnectButton:SetText("УБЕЖАТЬ");
				local newsizew, newsizeH = Clockwork.kernel:GetCachedTextSize(smallTextFont, "RUN IN FEAR");
				self.disconnectButton:SetPos((ScrW() / 2) - (newsizew / 2), ScrH() * 0.925);
				self.disconnectButton:SizeToContents();
			end;
...
function PANEL:Init()
	...
	self.faithLabel:SetText("Вера: "..faithText);
	...
	if self.customData then
		local zones_to_names = {
			...
			["manor"] = "Особняк",
			["scrapper"] = "Фабрика",
			["toothboy"] = "Его Обитель",
			["tower"] = "Башня Света",
			["wasteland"] = "Пустошь"
		};
		
		local zoneName = zones_to_names[self.customData.location] or "Пустота";
		
		self.locationLabel:SetText("Место: "..zoneName);
	else
		self.locationLabel:SetText("Место: За Реальностью");
	end
	
	self.sacramentsLabel = vgui.Create("DLabel", self);
	self.sacramentsLabel:SetText("Уровень: "..self.customData.level);
	...
	if self.customData.timesurvived then
		...
		self.timeSurvivedLabel:SetText("Прожил: "..FormattedPlayTime(self.customData.timesurvived, "%02i:%02i:%02i"));
	else
		self.timeSurvivedLabel:SetText("Прожил: 00:00:00");
	end
	...
	self.deleteButton:SetText("Забыть");
...
function PANEL:Init()
	...
	if self.customData.timesurvived then
		--self.timeSurvivedLabel:SetText("Time Survived: "..tostring(os.date("!%X", self.customData.timesurvived)));
		--self.timeSurvivedLabel:SetText("Time Survived: "..string.FormattedTime(self.customData.timesurvived, "%02i:%02i:%02i"));
		self.timeSurvivedLabel:SetText("Времени Прожито: "..FormattedPlayTime(self.customData.timesurvived, "%02i:%02i:%02i"));
	else
		self.timeSurvivedLabel:SetText("Времени Прожито: 00:00:00");
	end
	...
	if self.customData.deathcause and self.customData.deathcause ~= "" then
		...
		self.deathCauseLabel:SetText("Умер при загадочных обстоятельствах.");
	end
...
function PANEL:Init()
	...
	for k, v in pairs(Clockwork.class:GetAll()) do
		if (v.isOnCharScreen and (v.factions and table.HasValue(v.factions, self.info.faction))) then
			self.classTable = v;
			self.overrideData = {
				information = "Выбери класс своего персонажа.",
				Callback = function()
...
function PANEL:OnNext()
	if (!self.info.class or !Clockwork.class:FindByID(self.info.class)) then
		Clockwork.character:SetFault("Ты не выбрал класс! Или выбранный тобой класс недействителен!");
		return false;
	end;
end;
...
function PANEL:Init()
	...
	if (!Clockwork.faction.stored[self.info.faction].GetName) then
		...
		self.nameFormTitle:SetText("Имя");
	...
	if (self.bSelectModel or self.bPhysDesc) then
		...
		if (self.bPhysDesc) then
			self.appearanceFormHelp = self.appearanceForm:Help("Физическое описание вашего персонажа.");
		...
		if (self.bSelectModel) then
			...
			self.skinButton:SetText("Выбери Внешность");
	...
	if (self.bTraits) then
		...
		self.traitsFormTitle:SetText("Черты");
		...
		for k, v in SortedPairsByMemberValue(Clockwork.trait:GetAll(), "points") do
			...
			traitButton:SetToolTipCallback(function(frame)
				if frame then
					...
					if traitButton.disableTable then
						local disabledTraitsText = "Отключает ";
						...
						for i = 1, #traitButton.disableTable do
							...
							if i == #traitButton.disableTable - 1 then
								disabledTraitsText = disabledTraitsText..", и ";
					...
					if traitButton.eventlocked then
						frame:AddText("Сейчас эта черта недоступна.", selectedBad);
					elseif traitButton.excludedfactions and Clockwork.Client.SelectedFaction and table.HasValue(traitButton.excludedfactions, Clockwork.Client.SelectedFaction) then
						frame:AddText("Эта черта не может быть взята твоей фракцией.", selectedBad);
					elseif traitButton.excludedsubfactions and Clockwork.Client.SelectedSubfaction and table.HasValue(traitButton.excludedsubfactions, Clockwork.Client.SelectedSubfaction) then
						frame:AddText("Эта черта не может быть взята твоей подфракцией", selectedBad);
...
function PANEL:OnNext()
	...
	if (!Clockwork.faction.stored[self.info.faction].GetName) then
		if (IsValid(self.fullNameTextEntry)) then
			...
			if (self.info.fullName == "") then
				Clockwork.character:SetFault("Ты не ввел имя! Или же твое имя недопустимое.");
				return false;
			end;

			if (string.len(self.info.fullName) < 2) then
				Clockwork.character:SetFault("Твое имя должно быть длиной не менее 2 символов.!");
				return false;
			end;

			if (string.len(self.info.fullName) > 48) then
				Clockwork.character:SetFault("Длина твоего имени не должна превышать 48 символа!");
				return false;
			end;
		else
			...
			if (string.len(self.info.forename) < 2 or string.len(self.info.surname) < 2) then
				Clockwork.character:SetFault("Твое имя и Фамилия не могут быть такими короткими.");
				return false;
			end;
			
			if (string.len(self.info.forename) > 48 or string.len(self.info.surname) > 48) then
				Clockwork.character:SetFault("Твое имя и фамилия не должны превышать 48 символов.");
				return false;
			end;
		end;
	end;
	...
	if (self.bSelectModel and !self.info.model) then
		Clockwork.character:SetFault("Сначала выбери модель!");
		return false;
	end;
	...
	if (self.bPhysDesc) then
		...
		if (string.len(self.info.physDesc) < minimumPhysDesc) then
			Clockwork.character:SetFault("Физическое описание твоего персонажа должно быть не короче "..minimumPhysDesc.." символов!");
			return false;
		end;
	...
	if (self.bFaiths) then
		if not self.info.faith then
			Clockwork.character:SetFault("Ты не выбрал веру!");
			return false;
		end
	end
	...
	if (self.bTraits) then
		...
		if points < 0 then
			Clockwork.character:SetFault("Тебе не хватает очков особенностей!");
			return false;
		end
	end
	...
	if (self.bTraits) then
		...
		if points then
			...
			if (self.helpText) then
				...
				elseif(points < 0) then
					self.helpText:SetText("Ты потратил слишком много очков. Чтобы продолжить, тебе нужно выбрать негативных черт на "..math.abs(points).." очков."..addon);
				else
					self.helpText:SetText("У тебя закончились очки.");
				end
...
function PANEL:Init()
	...
	panel.nextButton:SetText("ДАЛЬШЕ");
	panel.nextButton:SizeToContents();
	panel.nextButton.Paint = function(panel, w, h)
		if (panel:GetHovered()) then
			local texts = {"ДаЛьШе", "дАлЬшЕ"};
...
function PANEL:OnNext()
	...
	Clockwork.character:SetFault("Ты не выбрал фракцию. Или твоя фракция недоступна!");
	return false;
end;
			...
			for i = 1, #factionTable.subfactions do
				...
				button.DoClick = function()
					...
					if factionTable.subfactions[i].rivalry then
						...
						subfactionRivalryText:SetText("Враждует с "..subfactionRivalry);
			...
			for i = 1, #factionTable.subfactions do
				...
				if subfaction.name == Clockwork.Client.SelectedSubfaction then
					if subfaction.locked then
						Clockwork.character:SetFault("Ты не можешь выбрать эту подфракцию!");
						return false;
					elseif subfaction.whitelist and !Clockwork.character:IsWhitelistedSubfaction(subfaction.name) then
						Clockwork.character:SetFault("У тебя нет вайтлиста этой подфракции!");
						return false;
					end
				end
			end

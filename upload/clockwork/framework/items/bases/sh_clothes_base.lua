...
local ITEM = item.New(nil, true);
	...
	function ITEM:OnPlayerUnequipped(player, extraData, bSkipProgressBar)
		...
		if extraData == "drop" then
			...
			if (player:GetShootPos():Distance(trace.HitPos) <= 192) then
				...
				Clockwork.player:Notify(player, "Ты не можешь выкинуть этот предмет так далеко!");
			end
		end
		...
		if Clockwork.equipment:UnequipItem(player, self, nil, !bSkipProgressBar) then
			...
			if action == "putting_on_armor" or action == "taking_off_armor" then
				Schema:EasyText(player, "peru", "Ты не можешь это сделать, ибо твои руки заняты!");
				return false;
			end
	...
	function ITEM:OnUse(player, itemEntity, bSkipProgressBar)
		...
		if cwPowerArmor and player:IsWearingPowerArmor() then
			Schema:EasyText(player, "peru", "Ты не можешь это сделать, ибо находишься в силовой броне!");
			return false;
		end
	
		if self:IsBroken() then
			Schema:EasyText(player, "peru", "Этот предмет сломан и не может быть использован!");
			return false;
		end
		...
		if self.hasHelmet then
			...
			if helmetItem then
				Schema:EasyText(player, "peru", "Вы не можете носить это, так как ваша броня имеет шлем в комплекте!")
				return false
			end
		end
			...
			if clothesItem then
				if clothesItem.attributes and table.HasValue(clothesItem.attributes, "not_unequippable") then
					if !self.attributes or !table.HasValue(self.attributes, "not_unequippable") then
						Schema:EasyText(player, "peru", "Ты не можешь снять броню, ибо она стала частью твоей плоти!")
						return false;
					end
				end
			end
			...
			if (!self.CanPlayerWear or self:CanPlayerWear(player, itemEntity) != false) then
				if itemEntity then
					if (Clockwork.entity:BelongsToAnotherCharacter(player, itemEntity)) then
						Schema:EasyText(player, "peru", "Ты не можешь подобрать предмет своего другого персонажа!");
					...
					Clockwork.player:SetAction(player, "putting_on_armor", actionTime, 1, function()
						if IsValid(player) and self then
							if !itemEntity or itemEntity and IsValid(itemEntity) then
								...
								Schema:EasyText(player, "peru", "Этот предмет недействителен!");
							end
						end
					end);

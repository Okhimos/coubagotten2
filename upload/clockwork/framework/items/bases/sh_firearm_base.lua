...
ITEM.requireFaction = {};
ITEM.breakable = true;
ITEM.breakMessage = " разлетается на куски!";
...
function ITEM:Engrave(player, text, engravingItemTable)
	...
	elseif #nospaces == 0 then
		...
		Schema:EasyText(player, "olivedrab", "Ты выгравировал \'"..text.."\' на стволе своего оружия.");
...
function ITEM:OnUse(player, itemEntity)
	...
	if (!player:HasWeapon(weaponClass)) then
		...
		Schema:EasyText(player, "peru", "У тебя уже есть оружие такого типа!")
...
function ITEM:OnEquip(player)
	if self:IsBroken() then
		Schema:EasyText(player, "peru", "Это оружие сломано и не может использоваться!");
		return false;
	end
	...
	Schema:EasyText(player, "peru", "У тебя должен быть свободный слот!")
	return false;
end

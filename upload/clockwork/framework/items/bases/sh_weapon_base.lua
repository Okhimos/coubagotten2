...
ITEM.breakable = true;
ITEM.breakMessage = " ломается пополам!";
...
function ITEM:Engrave(player, text, engravingItemTable)
	local nospaces = string.gsub(text,"%s+","")
		...
		Schema:EasyText(player, "peru", "Это название слишком длинное!");
	if string.utf8len(text) > 32 then
	elseif #nospaces == 0 then
		Schema:EasyText(player, "chocolate", "Это поле нельзя оставить пустым!");
	else
		self:SetData("engraving", text);
		player:TakeItem(engravingItemTable, true);
		Schema:EasyText(player, "olivedrab", "Ты выгравировал \'"..text.."\' на передней стороне "..self.name..".");
	...
	function ITEM:OnCustomFunction(player, name)
		if (name == "Engrave") then
			if self:GetData("engraving") != "" then
				Schema:EasyText(player, "peru", "Это оружие уже имеет Гравировку!");
			elseif cwBeliefs and !player:HasBelief("literacy") then
				...
				if engravingItemTable then
					Clockwork.dermaRequest:RequestString(player, "Engrave Weapon", "Какое название ты хочешь дать?", "", function(result)
						self:Engrave(player, result, engravingItemTable);
					end);
				else
					Schema:EasyText(player, "chocolate", "У тебя нету предмета, которым можно было бы оставить гравировку!");
					return false;
				end
			end;
		end;
...
function ITEM:OnUse(player, itemEntity, interactItemTable)
	...
	if (!player:HasWeapon(weaponClass)) or interactItemTable then
		...
		Schema:EasyText(player, "peru", "У тебя уже есть экипированное оружие такого типа!")
...
function ITEM:OnEquip(player, interactItemTable)
	if self:IsBroken() then
		Schema:EasyText(player, "peru", "Это оружие сломано и не может быть использовано!");
		return false;
	end
	...
	if interactItemTable then
		...
		if slot then
			if !player.equipmentSlots[slot] then
				Schema:EasyText(player, "peru", "Вы не можете экипировать оружие для второй руки в слот, в котором нет оружия!")
				return false;
			end
			...
			if shieldItem then
				...
				if numValidItems < 2 then
					...
					if !weaponItem or !weaponItem:IsTheSameAs(interactItemTable) then
						Schema:EasyText(player, "peru", "Вы не можете экипировать оружие для второй руки вместе с оружием, использующим щит!")
						return false;
					end
				end
			end
			...
			if !attackTable.canaltattack and attackTable.dmgtype == DMG_VEHICLE and !offhandAttackTable.canaltattack and offhandAttackTable.dmgtype ~= DMG_VEHICLE then
				Schema:EasyText(player, "peru", "Ты не можешь сделать это!");
				return false;
			...
			Schema:EasyText(player, "peru", "Вы не можете экипировать другое оружие в этот слот!")
			return false;
		end
		
		Schema:EasyText(player, "peru", "Не удалось найти подходящий слот для этого оружия!")
		return false;
	else

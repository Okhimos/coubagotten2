...
local ITEM = item.New(nil, true);
	...
	function ITEM:OnDrop(player, position)
		if (self:HasPlayerEquipped(player)) then
			if !player.spawning then
				Schema:EasyText(player, "peru", "Ты не можешь выкинуть предмет, что носишь на себе.")
			end
	...
	function ITEM:OnUse(player, itemEntity)
		if (self:HasPlayerEquipped(player)) then
			if !player.spawning then
				Schema:EasyText(player, "peru", "У тебя уже есть такой амулет.")
			end
		...
		if self.requireFaith and not (table.HasValue(self.requireFaith, player:GetFaith())) then
			if !self.kinisgerOverride or self.kinisgerOverride and !player:GetCharacterData("apostle_of_many_faces") then
				if !player.spawning then
					Schema:EasyText(player, "chocolate", "Твоя вера не позволяет использовать этот нечестивый амулет!")
				end
		...
		if self.requiredSubfaiths and not (table.HasValue(self.requiredSubfaiths, player:GetSubfaith())) then
			if !self.kinisgerOverride or self.kinisgerOverride and !player:GetCharacterData("apostle_of_many_faces") then
				if !player.spawning then
					Schema:EasyText(player, "chocolate", "Ты не можешь использовать этот амулет!")
				end
		...
		if self.mutuallyExclusive then
			for i, v in ipairs(self.mutuallyExclusive) do
				if player:GetCharmEquipped(v) then
					if !player.spawning then
						Schema:EasyText(player, "chocolate", "Этот амулет является взаимоисключающим с другим установленным амулетом!")
					end
		...
		if (player:Alive()) then
			...
			if !player.spawning then
				Schema:EasyText(player, "peru", "У тебя должно быть место на поясе, чтобы повесить амулет!")
			end

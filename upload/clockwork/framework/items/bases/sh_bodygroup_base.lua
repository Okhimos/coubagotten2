...
local ITEM = Clockwork.item:New(nil, true)
	...
	function ITEM:OnUse(player, itemEntity)
		...
		if cwPowerArmor and player:IsWearingPowerArmor() then
			Schema:EasyText(player, "peru", "Ты не можешь надеть это, ибо находишься в силовой броне!");
			return false;
		end
		
		if self:IsBroken() then
			Schema:EasyText(player, "peru", "Этот шлем сломан и не может быть использован!");
			return false;
		end
		...
		if clothesItem then
			if clothesItem.hasHelmet then
				Schema:EasyText(player, "peru", "У тебя есть шлем такого типа!")
				return false
			end
		end
		...
		if helmetItem then
			if helmetItem.attributes and table.HasValue(helmetItem.attributes, "not_unequippable") then
				if !self.attributes or !table.HasValue(self.attributes, "not_unequippable") then
					Schema:EasyText(player, "peru", "Твой нынешний шлем стал частью плоти и не может быть снят!");
					return false;
				end
			end
		end
		...
		if Clockwork.equipment:UnequipItem(player, self) then
			if extraData == "drop" then
				...
				if (player:GetShootPos():Distance(trace.HitPos) <= 192) then
					...
					Clockwork.player:Notify(player, "Ты не можешь выкинуть это так далеко!");
				end
			end

...
function Schema:PermaKillPlayer(player, ragdoll, bSilent)
	if player then
		...
		if player.GetCharmEquipped and player:GetCharmEquipped("satchel_denial") then
			...
			Clockwork.chatBox:AddInTargetRadius(player, "me", "исчезает на твоих глазах!", player:GetPos(), config.Get("talk_radius"):Get() * 2);
		end
...
function Schema:CheapleCaughtPlayer(player)
	if IsValid(player) and player:Alive() then
		...
		Clockwork.chatBox:Add(listeners, player, "me", "просто исчезает, как будто никогда и не существовал!", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
...
function Schema:BloodTestPlayer(player, bFalsePositives, bDetectImposters)
	...
	if (faith ~= "Faith of the Light" and ((!bDetectImposters and subfaction ~= "Kinisger") or bDetectImposters)) or subfaith == "Voltism" or (bFalsePositives and math.random(1, 20) == 1 and not player:HasBelief("favored")) then
		Clockwork.chatBox:AddInTargetRadius(player, "it", "Набор для проверки крови мигает красным.", player:GetPos(), config.Get("talk_radius"):Get() * 2);
	...
	elseif cwCharacterNeeds and player:GetNeed("corruption", 0) >= 50 and subfaction ~= "Kinisger" then
		...
		Clockwork.chatBox:AddInTargetRadius(player, "it", "Набор для проверки крови мигает зеленым.", player:GetPos(), config.Get("talk_radius"):Get() * 2);
...
netstream.Hook("QueryBountyBoard", function(player, state)
	if cwBeliefs and !player:HasBelief("literacy") then
		Schema:EasyText(player, "chocolate", "Вы смотрите на забавные картинки, что находятся на доске объявлений, но ничего не можете понять! Как странно...");
	else
		local tab = {};
...
function playerMeta:AddBounty(bounty, reason, poster)
	if IsValid(poster) and poster:IsPlayer() then
		...
		if (faction  == "Gatekeeper" or faction  == "Holy Hierarchy") and !poster:IsAdmin() then
			Schema:EasyText(poster, "cornflowerblue", "Ты не можешь вывесить информацию о "..self:Name().." на доску!");
	...
	if self:GetCharacterData("permakilled") or !self:Alive() then
		if IsValid(poster) and poster:IsPlayer() then
			Schema:EasyText(poster, "peru", self:Name().." мертв!");
		end
	...
	local physDesc = self:GetCharacterData("PhysDesc", "В грязной одежде и с маленькой сумочкой на боку.");
	...
	if IsValid(poster) and poster:IsPlayer() then
		Schema:EasyText(poster, "cornflowerblue", "Ты вывесил объявление "..tostring(tab.bounty).." за "..tab.name.."!");
...
function playerMeta:RemoveBounty(remover)
	...
	Schema:EasyText(remover, "peru", self:Name().." нету такого объявления!");
end;
			...
			if (Clockwork.database:IsResult(result)) then
				for k, v in pairs(result) do
					...
					if IsValid(poster) and poster:IsPlayer() and faction then
						if (faction  == "Gatekeeper" or faction  == "Holy Hierarchy") and !poster:IsAdmin() then
							Schema:EasyText(poster, "cornflowerblue", "Ты не можешь вывесить объявление о "..v._Name.."!");
					...
					if data and permakilled ~= true then
						...
						local physDesc = data["PhysDesc"] or "В грязной одежде и с маленькой сумкой на боку.";
...
function Schema:RemoveBounty(key, remover)
	...
	local queryObj = Clockwork.database:Select(charactersTable)
		queryObj:Callback(function(result)
			if (Clockwork.database:IsResult(result)) then
				for k, v in pairs(result) do
					if Schema.bountyData[v._Key] then
						...
						if IsValid(remover) and remover:IsPlayer() then
							Schema:EasyText(remover, "cornflowerblue", "Ты удалил объявление "..v._Name.."!");
			...
			if IsValid(remover) and remover:IsPlayer() then
				Schema:EasyText(remover, "peru", "Человек, которого вы хотите объявить "..tostring(key).." не найден и не может быть объявлен в розыск!");
			end
		end);
...
function Schema:AddBookTowerArchives(player, itemTable)
	...
	Schema:EasyText(player, "olivedrab", "Ты добавил копию '"..itemTable.name.."' в архив.");
end
...
function Schema:TakeBookTowerArchives(player, uniqueID)
	...
	if self.archivesBookList[uniqueID] and isnumber(self.archivesBookList[uniqueID]) then
		...
		if (bSuccess) then
			...
			Schema:EasyText(player, "olivedrab", "Ты взял копию '"..itemTable.name.."' из Архива.");
		...
		Schema:EasyText(player, "peru", "Этой книги нету в Архиве!");
		self.archivesBookList[uniqueID] = nil
	end
end
	...
	if (trace.Entity) then
		...
		if (entity:GetClass() == "cw_coinslot") then
			...
			if (faction ~= "Goreic Warrior") then
				...
				Schema:EasyText(player, "olive", "Вы тянете рычаг, чтобы проверить налог Башни Света. Согласно механическому дисплею кассы, текущая ставка составляет "..tax.."%.");
	...
	if (trace.Entity) then
		...
		if (entity:GetClass() == "cw_coinslot") then
			...
			if (faction == "Gatekeeper" or faction == "Pope Adyssa's Gatekeepers" or faction == "Holy Hierarchy") then
				...
				Schema:EasyText(player, "olive", "Вы тянете рычаг, чтобы проверить свою зарплату. Согласно механическому дисплею на кассе, у вас есть "..collectableWages.." заработные платы на сумму в  "..coin.." коинов.");
				
				if ranksRestrictedWages and ranksRestrictedWages[faction] and table.HasValue(ranksRestrictedWages[faction], rank) then
					Schema:EasyText(player, "peru", "Дисплей показывает, что пока ты находишься в башне, не будешь получать зарплату!");
				end
		...
		if (entity:GetClass() == "cw_coinslot") then
			...
			if (faction == "Gatekeeper" or faction == "Pope Adyssa's Gatekeepers" or faction == "Holy Hierarchy") then
				...
				if Schema.towerTreasury >= coin then
					...
					Schema:EasyText(player, "olivedrab", "Вы тянете за рычаг, чтобы получить зарплату. Машина выплевывает несколько монет на сумму в  "..coin.." коинов.");
					entity:EmitSound(coinslotSounds[math.random(#coinslotSounds)]);
			...
			if (faction ~= "Goreic Warrior") then
				...
				if (unixTime >= player:GetCharacterData("nextration", 0)) then
					if (Schema.towerTreasury and Schema.towerTreasury <= 250) or config.GetVal("enable_famine") then
						Schema:EasyText(player, "olive", "Вы тянете рычаг, чтобы получить провиант, но пайки не выдаются, хотя вы чувствуете, что прошло уже достаточно времени. Как странно.");
						entity:EmitSound(coinslotSounds[math.random(#coinslotSounds)]);
					...
					if (faction == "Gatekeeper" or faction == "Pope Adyssa's Gatekeepers" or faction == "Holy Hierarchy") then
						...
						Schema:EasyText(player, "olivedrab", "Машина со скрежетом выплевывает из себя бутылку воды и небольшой сверток. Просто прекрасно.");
						entity:EmitSound(coinslotSounds[math.random(#coinslotSounds)]);
					elseif player:HasBelief("favored") and math.random(1, 3) == 1 then
						...
						Schema:EasyText(player, "olivedrab", "Машина со скрежетом выдает половину буханки заплесневелого хлеба и бутылку грязной воды.");
						entity:EmitSound(coinslotSounds[math.random(#coinslotSounds)]);
					end
					
					player:SetCharacterData("nextration", unixTime + 7200);
				else
					Schema:EasyText(player, "olive", "Вы тянете рычаг пайка, но пища не выдается. По всей видимости придется подождать.");
					entity:EmitSound(coinslotSounds[math.random(#coinslotSounds)]);
				end;
			end
		end
				...
				if !collectedGear then
					...
					if (unixTime >= player:GetData("nextGear", 0)) then
						if (Schema.towerTreasury and Schema.towerTreasury <= 250) then
							Schema:EasyText(player, "olive", "Вы тянете рычаг, чтобы получить стандартный комплект снаряжения, но его не выдают. Как странно.");
							entity:EmitSound(coinslotSounds[math.random(#coinslotSounds)]);
						...
						Schema:EasyText(player, "olive", "Вы тянете рычаг, чтобы получить стандартный комплект снаряжения. Открывается приемное отверстие под машиной, и выдается грубая дорожная сумка.");
						entity:EmitSound(coinslotSounds[math.random(#coinslotSounds)]);
						...
						Schema:EasyText(player, "olive", "Вы тянете рычаг, чтобы получить набор снаряжения, но его не выдают. Похоже придется подождать.");
						entity:EmitSound(coinslotSounds[math.random(#coinslotSounds)]);
					end
				end;
			end
		...
		if (trace.Entity) then
			...
			if (entity:GetClass() == "cw_hellportal") then
				...
				if nextTeleport <= 0 then
					...
					Schema:EasyText(player, "peru", "Ты не сможешь использовать портал еще "..nextTeleport.." секунд!");
				end
			end
		end;
	end;

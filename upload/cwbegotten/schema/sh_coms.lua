...
local COMMAND = Clockwork.command:New("Enlist")
	COMMAND.tip = "Завербовать персонажа в свою фракцию. Это сработает только если ваш ранг может это делать."
		...
		if (target and target:Alive()) then
			if (target:GetShootPos():Distance(player:GetShootPos()) <= 192) then
				...
				if !enlistFactionTable.enlist then
					Schema:EasyText(player, "firebrick", "Он не может быть приглашен в  "..enlistFaction.." !");
				...
				if player:IsAdmin() or (enlistFaction == playerFaction and enlistFactionTable and enlistFactionTable.enlist and Schema:GetRankTier(playerFaction, player:GetCharacterData("rank", 1)) >= 3) or isMasterFaction then
					...
					if !Clockwork.faction:IsGenderValid(enlistFaction, target:GetGender()) then
						Schema:EasyText(player, "firebrick", target:Name().." не может оказаться в этой фракции, так как не того пола "..enlistFaction.." !");
					...
					if player:InTower() then
						if !Schema.towerSafeZoneEnabled then
							Schema:EasyText(player, "firebrick", "Безопасная Зона должна быть включена!");
							
							return;
						end
					else
						Schema:EasyText(player, "firebrick", "Ты должен находиться в Безопасном Месте!");
					...
					if !subfaction or istable(subfaction) then
						if targetFaction == "Wanderer" or (enlistFaction ~= "Children of Satan" and targetFaction == "Children of Satan" and target:GetSubfaction() == "Kinisger") then
							...
							Schema:EasyText(player, "green", "Ты пригласил "..target:Name().." в "..enlistFaction.." !");
						...
						elseif targetFaction == "Children of Satan" then
							...
							Schema:EasyText(target, "red", player:Name().." попытался пригласить тебя в  "..enlistFaction.." , но как Дитя Сатаны вы не можете его принять! Им было отправлено фальшивое сообщение о том, что вы приняли приглашение.");
							Schema:EasyText(player, "green", "You have invited "..target:Name().." to enlist into the "..enlistFaction.." faction!");
						else
							Schema:EasyText(player, "firebrick", target:Name().." не может быть рекрутирован в эту фракцию!");
						end
					else
						Schema:EasyText(player, "firebrick", subfaction.." не является допустимой подфракцией для этой фракции!");
					end
				else
					Schema:EasyText(player, "firebrick", "Вы не можете пригласить "..target:Name().." во фракцию!");
				end;
			else
	...
	function COMMAND:OnRun(player, arguments)
		...
		if (target) then
			...
			if player:IsAdmin() or ((playerFaction == faction and Schema:GetRankTier(playerFaction, player:GetCharacterData("rank", 1)) >= 3) or isMasterFaction) then
				...
				if (!ranks[faction]) then
					Schema:EasyText(player, "darkgrey", target:Name().." не может быть повышен!");
					return;
				end;
				...
				if (ranks[faction][rank]) then
					if rank == target:GetCharacterData("rank", 1) then
						Schema:EasyText(player, "grey", target:Name().." уже находится на ранге "..ranks[faction][rank].."!");
					...
					if !isMasterFaction and Schema:GetRankTier(faction, rank) >= Schema:GetRankTier(faction, player:GetCharacterData("rank", 1)) then
						if !player:IsAdmin() then
							Schema:EasyText(player, "grey", "Ты не можешь сменить ранг "..target:Name().." на "..ranks[faction][rank].."!");
					...
					if factionTable.CanPromote and factionTable:CanPromote(player, target, faction, targetSubfaction) == false then
						Schema:EasyText(player, "grey", "Ты не можешь сменить ранг "..target:Name().."!");
					...
					if (notifyTarget) then
						Schema:EasyText(target, "olivedrab", "Ты  был повышен на ранг \""..ranks[faction][rank].."\".")
					end;
					
					Schema:EasyText(player, "cornflowerblue", "Ты повысил "..name.." на ранг \""..ranks[faction][rank].."\".");
					...
					Schema:EasyText(player, "darkgrey", "Такого ранга не существует!");
				end;
			else
				Schema:EasyText(player, "grey", "У вас нет прав на изменение рангаf "..target:Name().."!");
			end;
		else
			Schema:EasyText(player, "grey", arguments[1].." это не действительный персонаж!");
		end;
	end
COMMAND:Register()
		...
		if (target) then
			...
			if player:IsAdmin() or ((playerFaction == faction and Schema:GetRankTier(playerFaction, player:GetCharacterData("rank", 1)) >= 3) or isMasterFaction) then
				...
				if !isMasterFaction and Schema:GetRankTier(faction, player:GetCharacterData("rank", 1)) <= Schema:GetRankTier(faction, target:GetCharacterData("rank", 1)) then
					if !player:IsAdmin() then
						Schema:EasyText(player, "grey", "Ты не можешь уволить "..target:Name().."!");
				...
				if factionTable.CanDemote and factionTable:CanDemote(player, target, faction, targetSubfaction) == false then
					Schema:EasyText(player, "grey", "Ты не можешь изменить ранг "..target:Name().."!");
				...
				if rankTier == 1 then
					Schema:EasyText(player, "grey", target:Name().." не может быть понижен!");
				...
				Schema:EasyText(player, "cornflowerblue", "Ты был понижен "..name.." на ранг \""..ranks[faction][rank].."\".");
				...
				Schema:EasyText(player, "grey", "Ты не можешь понизить "..target:Name().."!");
			end
		else
			Schema:EasyText(player, "grey", target.." недействительный персонаж!");
		end;
	end
COMMAND:Register()
							...
							if (!player.searching) then
								...
								if !player:IsNoClipping() and (!player.GetCharmEquipped or !player:GetCharmEquipped("urn_silence")) then
									...
									for k, v in pairs(ents.FindInSphere(player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2)) do
										if v:IsPlayer() then
											Clockwork.chatBox:Add(v, player, "me", "начинает обыскивать "..Clockwork.player:FormatRecognisedText(v, "%s", target)..".");
										end
									end
								end
								...
								Schema:EasyText(player, "peru", "Ты уже обыскиваешь!");
							end;
						else
							Schema:EasyText(player, "peru", "Ты не можешь обыскать того, кто двигается!");
						end;
					else
						Schema:EasyText(player, "peru", "Персонаж должен быть связан!");
					end;
				else
					Schema:EasyText(player, "peru", "Ты не можешь сделать это прямо сейчас!");
				end;
			else
				Schema:EasyText(player, "firebrick", "Он слишком далеко!");
			end;
		else
			Schema:EasyText(player, "peru", "Ты должен смотреть на персонажа!");
		end;
	end;
COMMAND:Register();
			...
			for k, v in pairs(players) do
				if IsValid(v) and v:HasInitialized() then
					...
					if vLastZone == "wasteland" or vLastZone == "tower" or vLastZone == "theater" then
						Clockwork.chatBox:Add(v, nil, "event", "Земля содрогается, когда ужасающий звук древного рога пронзает небеса!");
			...
			if entity:GetClass() == "cw_gorevillagehorn" then
				...
				if faction == "Goreic Warrior" then
					...
					if player.nextWarHorn and player.nextWarHorn > curTime then
						Schema:EasyText(player, "chocolate", "Ты должен подождать еще "..-math.ceil(curTime - player.nextWarHorn).." секунд прежде чем затрубить в рог снова!");
					...
					for _,v in pairs(_player.GetAll()) do
						...
						if (lastZone == "gore" or lastZone == "gore_tree" or lastZone == "gore_hallway") then
							if v:GetFaction() == "Goreic Warrior" then
								Clockwork.chatBox:Add(v, nil, "event", "Слышится ужасающий гул древнего рога разносящийся по всему лесу! Все воины Народа Копья должны оказаться в Центре Деревни!");
							else
								Clockwork.chatBox:Add(v, nil, "event", "Звук боевого рога разносится по всему лесу, но вы не не имеете представления о том, что это значит!");
							end
				...
				Schema:EasyText(player, "firebrick", "Ты должен смотреть на Горн, чтобы сделать это!");
			end
		end
	end;
COMMAND:Register();
		...
		if (trace.Entity) then
			...
			if entity:GetClass() == "cw_gorevillagehorn" then
				...
				if faction == "Goreic Warrior" then
					...
					if player.nextWarHorn and player.nextWarHorn > curTime then
						Schema:EasyText(player, "chocolate", "Ты должен подождать еще "..-math.ceil(curTime - player.nextWarHorn).." секунд, прежде чем трубить в свой рог!");
					...
					for _,v in pairs(_player.GetAll()) do
						...
						if (lastZone == "gore" or lastZone == "gore_tree" or lastZone == "gore_hallway") then
							if v:GetFaction() == "Goreic Warrior" then
								Clockwork.chatBox:Add(v, nil, "event", "Слышится ужасающий гул древнего рога разносящийся по всему лесу. Всем воинам, что отправляются в путь необходимо собраться в Центре Деревни.");
							else
								Clockwork.chatBox:Add(v, nil, "event", "Звук боевого рога разносится по всему лесу, но вы не не имеете представления о том, что это значит!");
							end
					...
					Schema:EasyText(player, "firebrick", "Ты не знаешь, как этим пользоваться!");
				end
			else
				Schema:EasyText(player, "firebrick", "Ты должен посмотреть на Горн, чтобы использовать его!");
			end
		end
	end;
COMMAND:Register();
	...
	function COMMAND:OnRun(player, arguments)
		...
		for i = 1, _player.GetCount() do
			...
			if IsValid(player) and player:HasInitialized() then
				...
				if lastZone == "wasteland" or lastZone == "tower" or lastZone == "theater" then
					Clockwork.chatBox:Add(player, nil, "event", "Неужели...? Нет, этого не может быть... Звучит сигнализация, это значит, что идет Ублюдок Джо...");
					netstream.Start(player, "FadeAmbientMusic");
...
local COMMAND = Clockwork.command:New("Proclaim");
	COMMAND.tip = "Произносите свои слова с тем вниманием, которого они заслуживают, если вы достаточно благородны.";
	COMMAND.text = "<string Text>";
	COMMAND.arguments = 1;
	...
	function COMMAND:OnRun(player, arguments)
		...
		if (text == "") then
			Clockwork.player:Notify(player, "Ты ничего не написал!");
...
COMMAND.tip = "Поговорите с помощью своего фамильяра с определенным человеком.";
		...
		if subfaction == "Clan Crast" then
			...
			if targetFaction == "Goreic Warrior" then
				if player:HasBelief("watchful_raven") then
					...
					Schema:EasyText(player, "firebrick", "Для этого вам необходимо иметь убеждение «Бдительный ворон»!");
				end
			else
				Schema:EasyText(player, "firebrick", target:Name().." не является Воином Горов!");
			end
		else
			Schema:EasyText(player, "firebrick", "Ты не можешь сделать это!");
		end
	else
		Schema:EasyText(player, "grey", arguments[1].." недействительный персонаж!");
	end
end;
...
local COMMAND = Clockwork.command:New("RavenSpeakClan");
	COMMAND.tip = "Поговорите с помощью своего фамильяра с другими членами клана Краст.";
...
local COMMAND = Clockwork.command:New("RavenSpeakFaction");
	COMMAND.tip = "Поговорите со всем Народом Копья при помощи Фамильяра Ворона.";
...
COMMAND.tip = "Поговорите с Воронами, чтобы передать сообщение члену клана Краст, который в последний раз говорил с вами при помощи Фамильяра.";
COMMAND.text = "<string Message>";
COMMAND.flags = CMD_DEFAULT;
COMMAND.arguments = 1;
...
function COMMAND:OnRun(player, arguments)
	if IsValid(player.lastRavenSpeaker) then
		...
		Schema:EasyText(player, "firebrick", "Рядом с тобой нету ворона, который мог бы донести послание!");
	end
end;
...
local COMMAND = Clockwork.command:New("ProclaimMe");
	COMMAND.tip = "Провозглашайте свои жесты с тем вниманием, которого они заслуживают, если вы достаточно благородны.";
	COMMAND.text = "<string Text>";
	COMMAND.arguments = 1;
	...
	function COMMAND:OnRun(player, arguments)
		...
		if (target) then
			...
			if target.possessor and target:GetNetVar("tied") != 0 then
				Clockwork.chatBox:AddInTargetRadius(target, "me", " путы с его конечностей внезапно падают на землю, словно их снимает какая-то невидимая сила!", target:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
			end
...
local COMMAND = Clockwork.command:New("HellJaunt");
	COMMAND.tip = "Возвращайтесь в Ад, используя темную магию, если вы Дитя Сатаны, хотя этот акт временно сделает вас уязвимым для влияния демонов и, таким образом, подвергнет вас сильному разложению. Любой, кого вы держите в руках, также будет телепортирован. Вы не можете совершать прыжок, если перегружены.";
	COMMAND.flags = CMD_DEFAULT;
	...
	function COMMAND:OnRun(player, arguments)
		if player:GetFaction() == "Children of Satan" then
			...
			if Schema.hellJauntDisabled or (map ~= "rp_begotten3" and map ~= "rp_begotten_redux") then
				Schema:EasyText(player, "peru", "Ваша связь с Адом, похоже, разорвана, и вы не можете отправиться в Ад!");
				
				return false;
			end
			
			if player.OverEncumbered then
				Schema:EasyText(player, "peru", "Вы не можете отправиться в ад пока перегружены!");
			...
			if player:GetNWBool("bliz_frozen") then
				Schema:EasyText(player, "peru", "Ты не можешь отправиться в ад, будучи превращенным в кусок льда!");
				
				return false;
			end
			
			if player:GetNWBool("PickingUpRagdoll") then
				Schema:EasyText(player, "peru", "Вы не можете совершить прыжок, пока поднимаешь тело!");
			...
			if IsValid(holdingEnt) and holdingEnt:GetClass() == "prop_ragdoll" then
				...
				if ragdollPlayer and ragdollPlayer:GetFaction() == "Children of Satan" and ragdollPlayer:Alive() and ragdollPlayer.OverEncumbered then
					Schema:EasyText(player, "peru", "Ты не можешь съебнуть в ад, держа на руках еще одно перегруженное Дитя Сатаны!");
			...
			if not player.opponent and not player:IsRagdolled() and player:GetNetVar("tied") == 0 then
				...
				if lastZone ~= "hell" and lastZone ~= "manor" then
					...
					if nextTeleport <= 0 then
							...
							if v:HasInitialized() and v:GetNetVar("yellowBanner") == true and v:Alive() then
								if v:GetMoveType() == MOVETYPE_WALK or v:IsRagdolled() or v:InVehicle() then
									if v:GetPos():Distance(player:GetPos()) <= 2048 then
										Schema:EasyText(player, "peru", "Ты чувствуешь, что есть смертный, использующий Желтое Знамя, что приковывает тебя к этому чертовому миру! Победи его, или сбеги как можно дальше.");
										Schema:EasyText(v, "peru", "Вы чувствуете, как ваше желтое знамя пульсирует энергией, когда попытка "..player:Name().." отправиться в ад не увенчалась успехом!");
						...
						Schema:EasyText(player, "peru", "Ты не можешь отправиться в ад еще "..nextTeleport.." секунд!");
					end
				else
					Schema:EasyText(player, "peru", "Ты уже в аду!");
				end
			else
				Schema:EasyText(player, "peru", "Ты не можешь отправиться в ад сейчас!");
			end
		else
			Schema:EasyText(player, "peru", "Ты не той фракции, чтобы сделать это!");
		end
	end;
COMMAND:Register();
...
local COMMAND = Clockwork.command:New("HellTeleport");
	COMMAND.tip = "Возвращайтесь в Ад, используя темную магию, если вы Дитя Сатаны, если вы находитесь рядом с Аркой Погибели или Столпами Творения, хотя это займет время. Любой, кто находится рядом с вами или находится в ваших руках, также будет телепортирован.";
	COMMAND.flags = CMD_DEFAULT;
	...
	function COMMAND:OnRun(player, arguments)
		if player:GetFaction() == "Children of Satan" then
			...
			if Schema.hellTeleportDisabled or !archPos or !pillarPos then
				Schema:EasyText(player, "peru", "Ваша связь с Адом, похоже, разорвана, и вы не можете туда телепортироваться!");
			...
			if not player.opponent and not player:IsRagdolled() and player:GetNetVar("tied") == 0 then
				...
				if lastZone ~= "hell" and lastZone ~= "manor" then
					...
					if origin:DistToSqr(archPos) <= (1600 * 1600) or origin:DistToSqr(pillarPos) <= (3000 * 3000) then
						...
						if nextTeleport <= 0 then
							Clockwork.chatBox:AddInTargetRadius(player, "me", "совершает темные колдунства, открывая портал в ад!", origin, Clockwork.config:Get("talk_radius"):Get() * 2);
									...
									if not player.opponent and not player:IsRagdolled() and player:GetNetVar("tied") == 0 then
										...
										if lastZone ~= "hell" and lastZone ~= "manor" then
											...
											if origin:DistToSqr(archPos) <= (1600 * 1600) or origin:DistToSqr(pillarPos) <= (3000 * 3000) then
												...
												Schema:EasyText(player, "red", "Вы начинаете телепортироваться в Ад из области, защищенной магией Темного Лорда, тем самым избавляя себя от порчи!");
											else
												Schema:EasyText(player, "peru", "Чтобы телепортироваться обратно в Ад, вы должны находиться рядом с Аркой Погибели или Столпами Творения!");
											end
										else
											Schema:EasyText(player, "peru", "Ты уже в аду!");
										end

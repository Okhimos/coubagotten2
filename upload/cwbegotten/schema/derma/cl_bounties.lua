...
function PANEL:Rebuild(bounties, state)
	...
	if !table.IsEmpty(bounties) then
		...
		descriptionLabel:SetText("Перечисленные здесь совершили преступления против Сияния и должны быть казнены.");
		...
		for k, v in SortedPairsByMemberValue(bounties, "bounty", true) do
			...
			if state == "Hierarchy" or Clockwork.Client:IsAdmin() then
				label.DoRightClick = function()
					Derma_Query("Ты хочешь убрать Объявление?", label:GetText(), "Да", function()
...
function PANEL:PaintOver(width, height)
	if not self.bounties or table.IsEmpty(self.bounties) then
		...
		draw.DrawText("Объявлений нету, но враги у Сияния все еще есть.", self.font, (width / 2), (height / 2), Color(170, 0, 0, 255), 1, 1);
	end
end
...
function PANEL:DisplayWantedPoster(bountyData, charKey)
	...
	proclamationLabel:SetText("ОБЪЯВЛЕНИЕ ОТ СВЯТОЙ ИЕРАРХИИ:");
	...
	rewardLabel:SetText(tostring(bountyData.bounty or 0).." НАГРАДА!");
	...
	captureLabel:SetText("ДОЛЖЕН БЫТЬ ЗАХВАЧЕН ЖИВЫМ ИЛИ МЕРТВЫМ!");
	...
	authorityLabel:SetText("Объявление вывешено по приказу:");

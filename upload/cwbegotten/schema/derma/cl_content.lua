...
function PANEL:Rebuild()
	...
	function self.disconnectButton:Paint(w, h)
		if (self:GetHovered()) then
			local texts = {"ОТКЛЮЧИТЬСЯ", "ОтКлЮчИтЬсЯ", "оТкЛюЧиТьСя"};
	...
	if self.missingWorkshop then
		...
		self.contentButton:SetText("КОНТЕНТ");
		...
		self.contentButton:SetCallback(function(panel)
			steamworks.ViewFile("3259685712")
		end);

		function self.contentButton:Paint(w, h)
			if (self:GetHovered()) then
				local texts = {"КОНТЕНТ", "КоНтЕнТ", "кОНтЕнТ"};
	...
	self.proceedButton:SetText("ПРОДОЛЖИТЬ");
	...
	function self.proceedButton:Paint(w, h)
		if (self:GetHovered()) then
			local texts = {"ПРОДОЛЖИТЬ", "пРоДоЛжИтЬ", "ПрОдОлЖиТь"};
...
function PANEL:Paint(w, h)
	...
	draw.SimpleText("У вас отсутствует часть или весь контент игрового режима!", bigTextFont, (w * 0.5) - (newsizew / 2), h * 0.4, colRed);
	...
	if self.missingMounts ~= "" then
		if self.missingWorkshop then
			...
			draw.SimpleText("Центральная кнопка перенаправит вас на страницу Мастерской Steam с нужнымы контентом.", smallTextFont, (w * 0.5) - (newsizew / 2), h * 0.61, colRed);
			...
			draw.SimpleText("В качестве альтернативы вы можете отключиться или продолжить игру на свой страх и риск.", smallTextFont, (w * 0.5) - (newsizew / 2), h * 0.71, colRed);
		else
			...
			draw.SimpleText("Кнопка по центру перенаправит вас на страницу Мастерской.", smallTextFont, (w * 0.5) - (newsizew / 2), h * 0.65, colRed);
		end
	elseif self.missingWorkshop then
		newsizew, newsizeH = Clockwork.kernel:GetCachedTextSize(smallTextFont, "The center button will take you to the content's Steam Workshop page.");
		draw.SimpleText("Центральная кнопка перенаправит вас на страницу Мастерской Steam с нужнымы контентом.", smallTextFont, (w * 0.5) - (newsizew / 2), h * 0.55, colRed);

		newsizew, newsizeH = Clockwork.kernel:GetCachedTextSize(smallTextFont, "Alternatively, you can disconnect or proceed onwards at your own risk.");
		draw.SimpleText("Центральная кнопка перенаправит вас на страницу Мастерской Steam с нужнымы контентом.", smallTextFont, (w * 0.5) - (newsizew / 2), h * 0.65, colRed);
	else

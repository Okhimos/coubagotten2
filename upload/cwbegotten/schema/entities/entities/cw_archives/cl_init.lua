...
function ENT:HUDPaintTargetID(x, y, alpha)
	...
	y = Clockwork.kernel:DrawInfo("Архивы", x, y, colorTargetID, alpha)
	y = Clockwork.kernel:DrawInfo("Великолепно высокая библиотечная полка, полная знаний.", x, y, colorWhite, alpha);
end;
...
local function CreateMenu(state)
	...
	local subMenu = menu:AddSubMenu("Добавить в Архив");
	...
	if Schema.archivesBookList and !table.IsEmpty(Schema.archivesBookList) then
		if Clockwork.Client:GetFaction() == "Holy Hierarchy" then
			subMenu = menu:AddSubMenu("Забрать из Архива...");

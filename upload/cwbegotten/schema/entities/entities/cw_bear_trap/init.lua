		...
		if entity:IsPlayer() or entity:IsNPC() or entity:IsNextBot() or Clockwork.entity:IsPlayerRagdoll(entity) then
			...
			elseif entity:IsPlayer() and entity:Alive() then
				if entity.teleporting then
					...
					Clockwork.chatBox:AddInTargetRadius(entity, "me", "как полный придурок встает в капкан, активируя его!", entity:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
					...
					if math.random(1, 2) == 1 then
						...
						if cwMedicalSystem and (!cwPowerArmor or !entity:IsWearingPowerArmor()) then
							...
							Clockwork.chatBox:AddInTargetRadius(entity, "me", " вскрикивает от того, что его левая нога ломается с ужасающим хрустом!", entity:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
						...
						if cwMedicalSystem and (!cwPowerArmor or !entity:IsWearingPowerArmor()) then
							...
							Clockwork.chatBox:AddInTargetRadius(entity, "me", "вскрикивает от того, что его правая нога ломается с ужасающим хрустом!", entity:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);

...
function ENT:HUDPaintTargetID(x, y, alpha)
	...
	y = Clockwork.kernel:DrawInfo("Доска с наградами за головы врагов Сияния", x, y, colorWhite, alpha);
end;
...
local function CreateMenu(state)
	...
	if state == "Hierarchy" then
		menu:AddOption("Add Bounty", function()
			Derma_StringRequest("Bounty Board", "Кого вы хотите объявить врагом?", nil, function(target)
				Derma_StringRequest(target, "Сколько будет стоить его голова?", nil, function(price)
					Derma_StringRequest(target, "Почему он должен быть убит?", nil, function(reason)

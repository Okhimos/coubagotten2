...
netstream.Hook("ObjectPhysDesc", function(data)
	...
	if (IsValid(entity)) then
		Derma_StringRequest("Description", "Каково физическое описание этого объекта?", nil, function(text)
...
netstream.Hook("Frequency", function(data)
	Derma_StringRequest("Frequency", "Какую частоту вы хотите установить?", data, function(text)
...
Clockwork.chatBox:RegisterClass("ravenspeak", "ic", function(info)
	Clockwork.chatBox:Add(info.filtered, nil, Color(159, 129, 112, 255), "Ворон садится к вам на плечо и говорит голосом "..info.speaker:Name().." "..info.text);
end);

Clockwork.chatBox:RegisterClass("ravenspeakclan", "ic", function(info)
	Clockwork.chatBox:Add(info.filtered, nil, Color(159, 129, 112, 255), "Стая воронов летает над вашей головой и головами других членов клана Краст, говоря голосом "..info.speaker:Name().." "..info.text);
end);

Clockwork.chatBox:RegisterClass("ravenspeakfaction", "ic", function(info)
	Clockwork.chatBox:Add(info.filtered, nil, Color(159, 129, 112, 255), "Стая воронов летает над твоей головой и головами других Горов, говоря голосом "..info.speaker:Name().." "..info.text);
end);
...
Clockwork.chatBox:RegisterDefaultClass("speaker", "ic", function(info)
	...
	Clockwork.chatBox:Add(info.filtered, nil, Color(123, 104, 238), "Голос "..info.name.." раздается по системе оповещения Башни Света и ее окресностей \""..info.text.."\"");
end);

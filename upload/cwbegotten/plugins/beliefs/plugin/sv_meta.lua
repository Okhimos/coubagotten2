...
function playerMeta:Cloak()
	...
	if !self.cloaked then
		Clockwork.chatBox:AddInTargetRadius(self, "me", "начинает исчезать, пока полностью не становится невидимым!", self:GetPos(), Clockwork.config:Get("talk_radius"):Get());
	end
...
function playerMeta:Uncloak()
	...
	if self.cloaked then
		Clockwork.chatBox:AddInTargetRadius(self, "me", "начинает материализовываться и становится видимым!", self:GetPos(), Clockwork.config:Get("talk_radius"):Get());
	end

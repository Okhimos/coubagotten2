					...
					for i, v in ipairs(_player.GetAll()) do
						if v:GetNetVar("yellowBanner") then
							if (v:GetPos():Distance(playerPos) <= config.Get("talk_radius"):Get()) then
								Schema:EasyText(player, "peru", "Есть один с поднятым желтым знаменем, рассеивающий вашу темную магию! Победите его или окажитесь как можно дальше!");
								Schema:EasyText(v, "peru", "Вы чувствуете, как ваше желтое знамя пульсирует энергией, когда магия "..player:Name().." сорвана и он оказывается виден всем вокруг!");
								player:Uncloak();
				...
				if player:Crouching() and player:GetNetVar("kinisgerCloak") == true and !plyTab.cwObserverMode then
					if !player.wOSIsRolling or !player:wOSIsRolling() then
						...
						elseif (plyTab.cloakCooldown - curTime) > 5 then
							Schema:EasyText(self.Owner, "chocolate", "Ты покрыт черным порохом и не можешь стать невидимым еще "..math.ceil(plyTab.cloakCooldown - curTime).." секунд!");
						else
							Schema:EasyText(self.Owner, "chocolate", "Ты не можешь стать невидимым еще "..math.ceil(plyTab.cloakCooldown - curTime).." секунд!");
						end
					end
				end
			end
...
function cwBeliefs:PlayerCanSwitchCharacter(player, character)
	if (player.cloaked) then
		return "Ты не можешь сменить персонажа, находясь в невидимости!";
	end;
end;
				...
				if IsValid(activeWeapon) and activeWeapon.isDagger or offhandWeapon and offhandWeapon.isDagger then
					if (!entity.mutilated or entity.mutilated < 3) then
						...
						if model == "models/animals/deer1.mdl" then
							Clockwork.chatBox:AddInTargetRadius(player, "me", "разделывает тело оленя, собирая его мясо.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
								...
								if IsValid(player) and IsValid(entity) then
									...
									if IsValid(activeWeapon) and activeWeapon.isDagger or offhandWeapon and offhandWeapon.isDagger then
										if (!entity.mutilated or entity.mutilated < 3) then
											...
											Clockwork.player:Notify(player, "В этом трупе не осталось мяса!");
										end
									end
								end
							end);
						elseif model == "models/animals/goat.mdl" then
							Clockwork.chatBox:AddInTargetRadius(player, "me", "разделывает тело козла, собирая его мясо.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
						...
						elseif model == "models/animal_ragd/piratecat_leopard.mdl" then
							Clockwork.chatBox:AddInTargetRadius(player, "me", "разделывает тело леопарда, собирая его мясо.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
						...
						elseif entity:GetNWEntity("Player"):IsPlayer() or entity:GetNWEntity("Player") == game.GetWorld() then
							Clockwork.chatBox:AddInTargetRadius(player, "me", "разрезает плоть на теле перед собой, собирая его мясо и кости.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
					...
					Clockwork.player:Notify(player, "У тебя должен быть кинжал!");
				end;
			end;
			...
			if (!entityPlayer or !entityPlayer:Alive()) then
				...
				if entity:GetNWEntity("Player"):IsPlayer() or entity:GetNWEntity("Player") == game.GetWorld() or table.HasValue(animalModels, model) then
					if (!entity.hearteaten) then
						...
						if model == "models/animals/deer1.mdl" then
							Clockwork.chatBox:AddInTargetRadius(player, "me", "вонзает руку в грудь оленя, вырывает его сердце и пожирает его целиком.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
						elseif model == "models/animals/goat.mdl" then
							Clockwork.chatBox:AddInTargetRadius(player, "me", "вонзает руку в грудь козла, вырывает его сердце и пожирает его целиком.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
						elseif model == "models/animal_ragd/piratecat_leopard.mdl" then
							Clockwork.chatBox:AddInTargetRadius(player, "me", "вонзает руку в грудь леопарда, вырывает его сердце и пожирает его целиком.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
						elseif model == "models/animals/bear.mdl" then
							Clockwork.chatBox:AddInTargetRadius(player, "me", "вонзает руку в грудь медведя, вырывает его сердце и целиком пожирает, смачно причмокивая.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
						else
							Clockwork.chatBox:AddInTargetRadius(player, "me", "вонзает руку в грудь бедолаги перед собой, вырывает его сердце и пожирает его целиком, смачно причмокивая.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
						end
		...
		elseif (arguments == "cwHarvestBones") then
			if (!entityPlayer or !entityPlayer:Alive()) then
				...
				if (!entity.bones or entity.bones < 4) then
					...
					if model == "models/animals/deer1.mdl" then
						Clockwork.chatBox:AddInTargetRadius(player, "me", "руками вгрызается в плоть оленя, вытаскивая его кости.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
					elseif model == "models/animals/goat.mdl" then
						Clockwork.chatBox:AddInTargetRadius(player, "me", "руками вгрызается в плоть козла, вытаскивая его кости.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
					elseif model == "models/animal_ragd/piratecat_leopard.mdl" then
						Clockwork.chatBox:AddInTargetRadius(player, "me", "руками вгрызается в плоть леопарда, вытаскивая его кости.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
					elseif model == "models/animals/bear.mdl" then
						Clockwork.chatBox:AddInTargetRadius(player, "me", "руками вгрызается в плоть медведя, вытаскивая его кости.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
					else
						Clockwork.chatBox:AddInTargetRadius(player, "me", "руками вгрызается в плоть тела перед собой, вытаскивая его кости.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
					end
					...
					Clockwork.player:Notify(player, "У этого тела не осталось пригодных костей!");
				end;
			end;
			...
			if table.HasValue(animalModels, entity:GetModel()) then
				...
				if IsValid(activeWeapon) and activeWeapon.isDagger or offhandWeapon and offhandWeapon.isDagger then
					if (!entity.skinned or entity.skinned < 1) then					
						...
						if model == "models/animals/deer1.mdl" then
							Clockwork.chatBox:AddInTargetRadius(player, "me", "начинает свежевать тушу оленя перед собой, собирая его мех и шкуру.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
						elseif model == "models/animals/goat.mdl" then
							Clockwork.chatBox:AddInTargetRadius(player, "me", "начинает свежевать тушу козла перед собой, собирая его мех и шкуру.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
						elseif model == "models/animal_ragd/piratecat_leopard.mdl" then
							Clockwork.chatBox:AddInTargetRadius(player, "me", "начинает свежевать тушу леопарда перед собой, собирая его мех и шкуру.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
						elseif model == "models/animals/bear.mdl" then
							Clockwork.chatBox:AddInTargetRadius(player, "me", "начинает свежевать тушу медведя перед собой, собирая его мех и шкуру.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
							...
							if IsValid(player) and IsValid(entity) then
								...
								if IsValid(activeWeapon) and activeWeapon.isDagger or offhandWeapon and offhandWeapon.isDagger then
									if (!entity.skinned or entity.skinned < 1) then
										...
										Clockwork.player:Notify(player, "У этого тела уже нету шкуры!");
									end
								end
							end
						end);
					...
					if !entity.iFrames then
						if IsValid(attacker) and attacker:IsPlayer() then
							for k, v in pairs(ents.FindInSphere(entity:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2)) do
								if v:IsPlayer() then
									Clockwork.chatBox:Add(v, attacker, "me", "ударяет"..Clockwork.player:FormatRecognisedText(v, "%s", entity).." но урон отражается невидимой силой!");
								end
							end
						else
							for k, v in pairs(ents.FindInSphere(entity:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2)) do
								if v:IsPlayer() then
									Clockwork.chatBox:Add(v, entity, "me", "ударяет, но его урон блокируется невидимой силой!");
								end
							end
						end
							...
							if entity:Alive() and entity:Health() < entity:GetMaxHealth() / 4 or entity:GetRagdollState() == RAGDOLL_FALLENOVER and originalDamage > 0 then
								...
								if entity:Health() - newDamage < 10 then
									...
									if hatred and hatred >= 100 then
										...
										local strikeText = "эффективно ударяет в";
										
										if attackerWeapon.isJavelin and attackerWeapon.Base ~= "sword_swepbase" then
											strikeText = "эффективно кидает кинжал в";
										end

										for k, v in pairs(ents.FindInSphere(entity:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2)) do
											if v:IsPlayer() then
												Clockwork.chatBox:Add(v, attacker, "me", strikeText.." нужную точку "..Clockwork.player:FormatRecognisedText(v, "%s", entity)..", но его ненависть настолько сильна, что он отказывается умирать!");
									...
									if itemTable and !entTab.distortedRingFiredDuel then
										if !cwRituals or (cwRituals and !entTab.scornificationismActive) or (!attacker:IsNPC() and !attacker:IsNextBot() and !attacker:IsPlayer()) then
											...
											for k, v in pairs(ents.FindInSphere(entity:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2)) do
												if v:IsPlayer() then
													Clockwork.chatBox:Add(v, attacker, "me", strikeText.." нужную точку "..Clockwork.player:FormatRecognisedText(v, "%s", entity)..", но кинжал в последний момент отскакивает!");
												end
											end
											
											Clockwork.player:Notify(entity, "Ваше кольцо ломается, выпаская огромное количество энергии и позволяя вам жить дальше!");
								...
								if entity.soulscorchActive then
									Clockwork.chatBox:AddInTargetRadius(attacker, "me", strikeText.." нужную точку "..entity:Name()..", гася его Святой Свет темной магией и мгновенно его убивая!", attacker:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
									...
									Clockwork.chatBox:AddInTargetRadius(attacker, "me", strikeText.." нужную точку "..entity:Name()..", мгновенно его убивая!", attacker:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
								end
					...
					if lastZone == "wasteland" and ((cwDayNight and cwDayNight.currentCycle == "night") or (cwWeather and cwWeather.weather == "bloodstorm")) then
						if cwMedicalSystem and newDamage >= 25 then
							...
							if rand == 1 then
								...
								Clockwork.player:Notify(attacker, "Ты чувствуешь, как сила Кровавой Луны течет сквозь твою душу! Твой смертный сосуд внезапно перестает кровоточить.");
							elseif rand == 5 then
								attacker:ResetInjuries();
								Clockwork.player:Notify(attacker, "Ты чувствуешь, как сила Кровавой Луны течет сквозь твою душу! Твой смертный сосуд внезапно исцеляет все травмы.");
							end
						end
					end
				end
		...
		if entity:Health() - damage < 10 then
			...
			if hatred and hatred >= 100 then
				if !cwRituals or (cwRituals and !entTab.scornificationismActive) or (!attacker:IsNPC() and !attacker:IsNextBot() and !attacker:IsPlayer()) then
					...
					Clockwork.chatBox:AddInTargetRadius(entity, "me", "в ярости кричит, отказываясь умирать!", entity:GetPos(), config.Get("talk_radius"):Get() * 2);
					if entity:GetGender() == GENDER_MALE then
						entity:EmitSound("misc/attack_01.ogg", 90, math.random(55,70))
					else
						entity:EmitSound("misc/attack_01.ogg", 90, math.random(100,120))
...
local decapitationStrings = {
	"жестоко обезглавливает",
	"обезглавливает",
	"отрубает голову",
};

local decapitationSuffixes = {
	"отправляя его голову в полет",
	"отправляя его голову в воздух",
	"оставляя кровавый ошметок",
};
					...
					timer.Create("DecapitationBuffTimer_"..entIndex, 20, 1, function()
						if IsValid(attacker) then
							if attacker.decapitationBuff then
								...
								Clockwork.hint:Send(attacker, "Бафф от обезглавливания прошел...", 10, Color(175, 100, 100), true, true);
							end
						end
					end);
		...
		for i = 1, #itemTable.requiredbeliefs do
			...
			if !player:HasBelief(belief) then
				if !itemTable.kinisgerOverride or itemTable.kinisgerOverride and !player:GetCharacterData("apostle_of_many_faces") then
					Clockwork.player:Notify(player, "Вы не можете экипировать этот предмет, так как у вас отсутствует нужное верование!");
					return false;
				end
			end;
		end;

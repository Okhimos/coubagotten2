...
Clockwork.chatBox:RegisterClass("prayerreply", "ic", function(info)
	Clockwork.chatBox:Add(info.filtered, nil, Color(255, 255, 153, 255), "Мощный голос отдается эхом в вашей голове "..info.text);
end);

Clockwork.chatBox:RegisterClass("darkwhisper", "ic", function(info)
	Clockwork.chatBox:Add(info.filtered, nil, Color(150, 20, 20, 255), info.speaker:Name().." говорит с тобой и другими детьми при помощи нечестивого шепота "..info.text);
end);
...
Clockwork.chatBox:RegisterClass("darkwhisperglobalkinisger", "ic", function(info)
	Clockwork.chatBox:Add(info.filtered, nil, Color(150, 20, 20, 255), "["..info.speaker:GetFaction().."] "..info.speaker:Name().." говорит с тобой и другими детьми при помощи нечестивого шепота "..info.text);
end);

Clockwork.chatBox:RegisterClass("darkwhisperevent", "ic", function(info)
	Clockwork.chatBox:Add(info.filtered, nil, Color(150, 20, 20, 255), "Дьявольский шепот пронзает ваш разум, сопровождаемый нечестивыми воплями: "..info.text);
end);

Clockwork.chatBox:RegisterClass("darkwhispernondark", "ic", function(info)
	Clockwork.chatBox:Add(info.filtered, nil, Color(150, 20, 20, 255), "Вы слышите ужасающий нечестивый шепот, что проходит к вам в голову "..info.text..". Вы можете ответить, используя /darkreply, но это немного повысит вашу порчу.");
end);

Clockwork.chatBox:RegisterClass("darkwhisperreply", "ic", function(info)
	Clockwork.chatBox:Add(info.filtered, nil, Color(150, 20, 20, 255), info.speaker:Name().." шепчет в пустоту, отвечая "..info.text);
end);
...
config.AddToSystem("Maximum Sacrament Level", "max_sac_level", "Максимальный уровень, до которого может подняться персонаж с помощью системы Убеждений.", 30, 80);
		...
		if player_has_belief or subfaction == "Clan Grock" then	
			...
			if (!player.lastWarCry) or player.lastWarCry < curTime then
				...
				if subfaction == "Clan Grock" then
					...
					Clockwork.chatBox:AddInTargetRadius(player, "me", "издает животный рык!", playerPos, radius);
				...
				Schema:EasyText(player, "firebrick", "Ты не можешь использовать боевой клич еще "..tostring(math.ceil(player.lastWarCry - curTime)).." секунд!");
			end
		else
			Schema:EasyText(player, "firebrick", "Пока ты не можешь этого сделать!");
		end
	end;
COMMAND:Register();
		...
		if player:HasBelief("voltism") then
			if player:GetNetVar("tied") == 0 and !player:IsRagdolled() then
				...
				if techItemTable then
					...
					Clockwork.chatBox:AddInTargetRadius(player, "me", "дергается, пока через его тело проходит поток электрического тока!", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
					
					player.nextFlagellate = curTime + 1;
				else
					Schema:EasyText(player, "chocolate", "У тебя нет микросхемы!");
				end
			else
		...
		if player:HasBelief("flagellant") or player:GetSubfaction() == "Kinisger" then
			if player:GetNetVar("tied") == 0 and !player:IsRagdolled() then
					...
					if activeWeapon:GetClass() == "cw_lantern" and player:HasBelief("extinctionist") then
						...
						if weaponItemTable then
							...
							if currentOil >= 1 then
								...
								Clockwork.chatBox:AddInTargetRadius(player, "me", "бичует "..selfless.." при помощи лампы заженной лампы, покрываясь пламенем!", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
								...
								Clockwork.chatBox:AddInTargetRadius(player, "me", "бичует "..selfless.."!", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
				...
				if player:GetNWBool("Guardening") then
					Schema:EasyText(player, "firebrick", "Ты не можешь ударить себя во время блока!");
					return false;
				end
				...
				if attacktable then
					...
					if activeWeapon.IsBellHammer then
						...
						Clockwork.chatBox:AddInTargetRadius(player, "me", "бичует "..selfless.." при помощи огромного колола, повергая окружающих в шок !", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
					else
						Clockwork.chatBox:AddInTargetRadius(player, "me", "бичует "..selfless.."!", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
					end
			...
			Schema:EasyText(player, "firebrick", "У тебя не хватает силы духа!");
		end
		
		player.nextFlagellate = curTime + 1;
	else
		Schema:EasyText(player, "firebrick", "Ты не можешь ударить себя!!");
	end
end;
...
function COMMAND:OnRun(player, arguments)
	if player:HasBelief("sol_orthodoxy") then
		if player:GetNetVar("tied") == 0 and !player:IsRagdolled() then
			...
			Schema:EasyText(player, "firebrick", "Нет-Нет. Ты будешь страдать!");
		end;
	else
		Schema:EasyText(player, "firebrick", "Ты не можешь сделать это сейчас!");
	end
end;

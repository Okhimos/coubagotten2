...
function cwRecipes:PlayerCanCraft(player, uniqueID, craftAmount)
	...
	if Clockwork.player:GetAction(player) or player:IsRagdolled() or !player:Alive() or player.opponent or (cwDueling and cwDueling:PlayerIsInMatchmaking(player)) or player:GetNetVar("tied") != 0 then
		Schema:EasyText(player, "lightslategrey", "Ты не можешь сделать это сейчас!");
		return false;
	end
			...
			if requiredBeliefs then
				for i = 1, #requiredBeliefs do
					if !player:HasBelief(requiredBeliefs[i]) then
						...
						Schema:EasyText(player, "lightslategrey", "Тебе нужно убеждение '"..cwBeliefs:GetBeliefName(recipeTable.requiredBeliefs[i]).."' чтобы создать этот предмет!");
						return false;
					end
				end
			end
	...
	if requiresHeatSource then
		...
		if not fire_found then
			Schema:EasyText(player, "firebrick", "Чтобы приготовить это, тебе нужен огонь!");
			return false;
		end
	end
	...
	if requiresSmithy then
		...
		if not valid_smithy_found then
			Schema:EasyText(player, "firebrick", "Тебе нужна кузня!");
			return false;
		end
	end
...
function cwRecipes:Craft(player, uniqueID, itemIDs, craftAmount)
	...
	if (IsValid(player) and uniqueID and isstring(uniqueID)) then
		if (!player.cwNextCraft or player.cwNextCraft < curTime) then
				...
				if self:PlayerMeetsCraftingItemRequirements(player, recipeTable, itemIDs, false, craftAmount) then
					if (recipeTable.craftTime) then
						...
						if (craftVerb != "") then
							...
							Clockwork.chatBox:AddInTargetRadius(player, "me", "начинает создавать "..craftName..".", player:GetPos(), config.Get("talk_radius"):Get() * 2);
						end;
			...
			player:Notify("Ты должен подождать еще "..-math.ceil(curTime - player.cwNextCraft).." прежде чем начать новый крафт!");
		end;
	end
end;
...
function cwRecipes:PlayerMeetsCraftingItemRequirements(player, recipeTable, itemIDs, bTake, craftAmount)
	if !itemIDs or table.IsEmpty(itemIDs) then
		Schema:EasyText(player, "lightslategrey", "Ты не выбрал предметы для крафта!");
		return false;
	end
	...
	if isstring(recipeTable) then
		...
		if !recipeTable or isstring(recipeTable) then
			Schema:EasyText(player, "lightslategrey", "Ты не выбрал рецепт или у тебя не хватает предметов!");
			return false;
		end
	end
		...
		for k, v in pairs(requirements) do
			...
			for i = 1, amount do
				...
				if not goods_found then
					Schema:EasyText(player, "lightslategrey", "Твои предметы не соответствуют требованиям выбранного рецепта!");
					return false;
				end
			end
		end

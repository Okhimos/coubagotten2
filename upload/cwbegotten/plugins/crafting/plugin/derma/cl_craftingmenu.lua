...
function PANEL:Rebuild()
	...
	if (!self.categoryList) then
		...
		self.weaponButton:SetFont("Civ5Tooltip5");
		...
		self.munitionButton:SetFont("Civ5Tooltip5");
		...
		self.armorButton:SetFont("Civ5Tooltip5");
		...
		self.medicalButton:SetFont("Civ5Tooltip5");
		...
		self.otherButton:SetFont("Civ5Tooltip5");
...
function PANEL:Rebuild()
	...
	if requiredBeliefs then
		...
		if #requiredBeliefNames == 1 then
			self.tierLabel:SetText("Необходимое Убеждение: '"..requiredBeliefNames[1].."'");
		else
	...
	if recipeData.requiresHeatSource then
		...
		self.heatSourceLabel:SetText("Необходим Огонь");
	...
	elseif recipeData.requiresSmithy then
		...
		self.smithyLabel:SetText("Необходима Кузня");
...
function PANEL:Init()
	...
	self.craftButton:SetText("Создать");
	...
	if (!self.selectedRecipeLabel) then
		...
		if !self:GetParent().selectedRecipe then
			self.selectedRecipeLabel:SetText("Ты не выбрал рецепт!");
		else

		...
		if damageInfo:IsDamageType(128) and damageInfo:GetDamage() >= 15 then
			...
			if self.strikesRequired <= 0 then
				...
				if math.random(1, 150) == 1 then
					...
					Clockwork.chatBox:AddInTargetRadius(player, "it", "Продолжая рудокопить, вы замечаете слабое мерцание. Ваши глаза обманывают вас? Нет! Это Золото!.", player:GetPos(), config.Get("talk_radius"):Get() * 2);
				elseif math.random(1, 666) == 1 then
					itemName = "uncut_blood_diamond";
					
					Clockwork.chatBox:AddInTargetRadius(player, "it", "Продолжая рудокопить в поте лица вы понимаете что нашли что-то полезное. Из кучи вытаскивается прекрасный кроваво-красный драгоценный камень. Шепот наполняет пещеры вокруг вас, когда рубин мерцает.", player:GetPos(), config.Get("talk_radius"):Get() * 2);
			...
			if self.oreLeft <= 0 then
				Clockwork.chatBox:AddInTargetRadius(player, "it", "Рудная жила пустеет, ее ресурсы полностью извлечены.", player:GetPos(), config.Get("talk_radius"):Get() * 2);

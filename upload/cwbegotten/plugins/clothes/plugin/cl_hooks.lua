...
function PLUGIN:GetProgressBarInfoAction(action, percentage)
	if (action == "putting_on_armor") then
		return {text = "Ты надеваешь броню. Нажми ЛКМ, чтобы отменить.", percentage = percentage, flash = percentage < 10};
	elseif (action == "taking_off_armor") then
		return {text = "Ты снимаешь броню. Нажми ЛКМ, чтобы отменить.", percentage = percentage, flash = percentage < 10};
	end
end

	...
	cwWeather.weatherTypes = {
		["acidrain"] = {
			...
			leadupCallback = function()
				...
				Clockwork.chatBox:Add(playersInWeatherZones, nil, "event", "Едкий запах медленно начинает распространяться по Пустоши, что-то грядет...");
			end,
			leadupTime = 60,
		},
		["ash"] = {
		...
		["bloodstorm"] = {
			...
			leadupCallback = function()
				...
				Clockwork.chatBox:Add(playersInWeatherZones, nil, "event", "Отдаленные вопли порожденных разносятся по всей Долине Рока. Что-то злое надвигается...");
			end,
			leadupTime = 60,
		},
		["fog"] = {

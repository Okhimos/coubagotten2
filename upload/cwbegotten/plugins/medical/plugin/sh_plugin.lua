...
local COMMAND = Clockwork.command:New("CharDiagnose");
	...
	function COMMAND:OnRun(player, arguments)
		if cwBeliefs and !player:HasBelief("doctor") then
			Schema:EasyText(player, "chocolate","У тебя нету убеждения, которое позволяет это сделать!");
			return false;
		end
			...
			if (target:GetShootPos():Distance(player:GetShootPos()) <= 192) then
				...
				if target:Alive() then
					...
					if target.bleeding then
						...
						if #bleedingLimbs > 0 then
							if #bleedingLimbs == 1 then
								diagnoseString = diagnoseString.." Он имеет кровотечение из "..bleedingLimbs[1]..".";
							else
								diagnoseString = diagnoseString.." Он имеет кровотечение из ";
					...
					if bloodLevel < cwMedicalSystem.maxBloodLevel - 1250 then
						diagnoseString = diagnoseString.." Он выглядит очень бледным. Похоже это из-за кровопотери.";
					...
					for i = 1, #symptoms do
						...
						elseif symptom == "Pustules" then
							if symptomText then
								symptomText = symptomText.." Он покрыт пустулами и бубонами — типичным симптомом чумы порожденного.";
							else
								symptomText = " Он покрыт пустулами и бубонами — типичным симптомом чумы порожденного.";
							end
						elseif symptom == "Deformities" then
							if symptomText then
								symptomText = symptomText.." Его кожа деформирована, а глаза выпучены.";
							else
								symptomText = "Его кожа деформирована, а глаза выпучены.";
							end
						...
						elseif symptom == "Nausea" then
							if symptomText then
								symptomText = symptomText.." Он бледен и дизориентирован. Это похоже на тошноту.";
							else
								symptomText = " Он бледен и дизориентирован. Это похоже на тошноту.";
							end
						end
					...
					if diagnoseString == "Ты внимательно осматриваешь "..target:Name().."." then
						diagnoseString = diagnoseString.." Он выглядит совершенно здоровым.";
					end
					
					Schema:EasyText(player, Color(255 * textColorScale, 255 * (1 - textColorScale), 0), diagnoseString);
				else
					Schema:EasyText(player, "indianred", diagnoseString.." По всей видимости он мертв и ты ничего не можешь сделать.");
				end
			else
				Schema:EasyText(player, "firebrick", "Он слишком далеко!");
			end;
		else
			Schema:EasyText(player, "firebrick", "Ты должен смотреть хоть на кого-то!");
		end;
	end;
COMMAND:Register();
...
function COMMAND:OnRun(player, arguments)
	...
	if (target) then
		...
		if !arguments[2] then
			local strings = {"внезапно изрыгает зеленую жижу на землю, забрызгивая округу!", "блюет на землю!", "давится, а затем раскрывает пасть и блюет во все стороны!"};
			...
			timer.Simple(3, function()
				if IsValid(target) then
					...
					if target:Alive() then
						local curse_strings = {"Бля-я-ядь...", "Горло..", "Дерьмо!", "Ублюдство...", "Угх..."};
			...
			local strings = {"внезапно раскрывает рот и выблевывает сгусток крови!", "давится, а после изрыгает кровь!", "закашливается, а через пару секунд блюет кровью!"};
			...
			timer.Simple(3, function()
				if IsValid(target) then
					...
					if target:Alive() then
						local curse_strings = {"О нет...", "Дерьмо...", "Мне пиздец?!", "Какого...Блядь.", "Угх..."};

		...
		if !player.GetCharmEquipped or !player:GetCharmEquipped("boot_contortionist") then
			...
			if bone_broken then
				if both_bones_broken then
					Clockwork.chatBox:AddInTargetRadius(player, "me", "ноги ломаются с ужасающим хрустом!", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
				else
					Clockwork.chatBox:AddInTargetRadius(player, "me", " "..bone_broken.." нога ломается с ужасающим хрустом!", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
				end

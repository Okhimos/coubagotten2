	...
	if need and table.HasValue(cwCharacterNeeds.Needs, need) then
		...
		if newAmount > currentAmount then
			...
			if need == "hunger" then
				...
				if newAmount >= 100 then
					if !self:IsRagdolled() then
						Clockwork.chatBox:Add(self, nil, "itnofake", "Я так больше не могу...");
			...
			elseif need == "thirst" then
				...
				if newAmount >= 100 then
					if !self:IsRagdolled() then
						Clockwork.chatBox:Add(self, nil, "itnofake", "Я так больше не протяну...");
						Clockwork.player:SetRagdollState(self, RAGDOLL_KNOCKEDOUT, 20);
						...
						if health > health - (maxHealth / 4) then
							...
							self:DeathCauseOverride("Умер от жажды.");
							self:Kill();
						end
			...
			elseif need == "corruption" then
				...
				if newAmount >= 100 then
					...
					if cwPossession then
						...
						self:DeathCauseOverride("Превратился в ебучего демона!.");
				...
				elseif newAmount >= 90 and currentAmount < 90 then
					Clockwork.chatBox:Add(self, nil, "itnofake", "Крики в голове становятся невыносимыми! ЧТО-ТО СРОЧНО НУЖНО ДЕЛАТЬ!");
				...
				elseif newAmount >= 66 and currentAmount < 66 then
					Clockwork.chatBox:Add(self, nil, "itnofake", "Нечто злое терзает твою душу...");
				end;
			end
		end

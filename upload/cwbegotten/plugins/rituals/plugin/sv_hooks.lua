	...
	if !player.opponent then
		...
		if player.soulscorchActive then
			...
			if (lastZone == "tower" or lastZone == "theater") and Schema.towerSafeZoneEnabled then
				Clockwork.chatBox:AddInTargetRadius(player, "me", "излучает святой свет из святой груди.", player:GetPos(), config.Get("talk_radius"):Get() * 2);
			else
				...
				Clockwork.chatBox:AddInTargetRadius(player, "me", "взрывается святым огненным шаром!", player:GetPos(), config.Get("talk_radius"):Get() * 2);

				...
				if self.Owner:Crouching() then
					if !self.Owner.cloaked then
						...
						elseif (self.Owner.cloakCooldown - curTime) > 5 then
							Schema:EasyText(self.Owner, "chocolate", "Ты покрыт черным порохом и не можешь замаскироваться еще "..math.ceil(self.Owner.cloakCooldown - curTime).." секунд!");
						else
							Schema:EasyText(self.Owner, "chocolate", "Ты не сможешь замаскироваться еще "..math.ceil(self.Owner.cloakCooldown - curTime).." секунд!");
						end
					else
						self.Owner:Uncloak();
					end
				else
					Schema:EasyText(self.Owner, "chocolate", "Чтобы включить маскировку, вам необходимо присесть.");
				end
			else
				Schema:EasyText(self.Owner, "peru", "Для включения маскировки вы должны находиться в пустоши или шахтах, а также в это время должна быть ночь или кровавый шторм (если вы на поверхности).");
			end

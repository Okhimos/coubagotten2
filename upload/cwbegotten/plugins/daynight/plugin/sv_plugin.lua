...
cwDayNight.cycles = {
	...
	["daytonight"] = {
		...
		nextCycle = "night",
		eventText = "Небо начинает темнеть, и по всей пустоши раздается звон колоколов, возвещая о немедленной необходимости найти укрытие. Наступает ночь.",
	...
	["nighttoday"] = {
		length = 300, -- 5 minutes in seconds.
		nextCycle = "day",
		eventText = "Кровавая Луна наконец-то скрывается за горизонтом и небо начинает светлеть. Вы пережили еще одну ночь.",
		fadeMusic = true
	},
};
	...
	for i = 1, playerCount do
		...
		if IsValid(player) and player:HasInitialized() then
			...
			elseif player:GetSubfaction() == "Rekh-khet-sa" then
				if cycle == "night" then
					Clockwork.chatBox:Add(player, nil, "event", "Чувствуешь, как поднимается Кровавая Луна. Пора действовать.");
				elseif cycle == "nighttoday" then
					Clockwork.chatBox:Add(player, nil, "event", "Кровавая Луна угасает, и ты понимаешь, что пора вернуться домой.");
				end
			end
		end
	end

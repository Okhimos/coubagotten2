								...
								if player:GetEyeTrace().HitSky then
									...
									if --[[!player:HasBelief("lunar_repudiation") and]] player:GetSanity() <= 0 then
										...
										if IsValid(ragdollEntity) and ragdollEntity.cwIsBeingHeld then
											...
											Schema:EasyText(player, "maroon", "Луна — это всё. Больше нет смысла.");
...
netstream.Hook("ShadowDamage", function(player, data)
	...
	Clockwork.chatBox:Add(listeners, player, "me", "начинает истошно вопить и бить "..selfless.."!", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
end)
...
function cwSailing:BeginSailing(longshipEnt, destination, caller)
	...
	if IsValid(owner) and caller == owner then
		...
		if IsValid(tr.Entity) and tr.Entity == longshipEnt then
			...
			Clockwork.chatBox:AddInTargetRadius(owner, "me", "готовит судно к отправке в "..tostring(self.shipDestinations[destination].name)..".", owner:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
						...
						if longshipEnt.destination == "hell" then
							...
							for k, v in ipairs(_player.GetAll()) do
								if v:GetFaction() == "Children of Satan" and v:Alive() then
									...
									Schema:EasyText(v, "red", "Мимо вас проносится подавляющий порыв адского ветра, разносящий шепот проклятых душ, освободившихся от страданий. Границы владений Темного Лорда пересечены воиском с севера! Вскоре они обрушатся на Поместье.");
									v:HandleSanity(-15);
								end
							end
									...
									if entity:GetPos():Distance(player:GetPos()) < 256 then
										...
										if IsValid(activeWeapon) and activeWeapon:GetClass() == "cw_lantern" then
											...
											if oil >= 1 and player:IsWeaponRaised(activeWeapon) then
												...
												Schema:EasyText(player, "peru", "Ваш фонарь должен быть зажжен, чтобы поджечь корабль!");
											end
										end
									end
								end
	...
	if (trace.Entity) then
		...
		if IsValid(entity) and (entity:GetClass() == "cw_longship" or entity:GetClass() == "cw_longship_ironclad") then
			...
			if entity:GetClass() == "cw_longship_ironclad" then
				status_string = "Шагалаксианский стальной бегемот, работающий на паровом двигателе.";
				
				if entity.machinegun then
					status_string = status_string.." На носу установлен пулемет..";
				end
			end
			...
			if entity.health then
				...
				if entity:GetSkin() == 1 then
					if health >= 1000 then
						status_string = status_string.."Корабль в прекрасном состоянии.";
					elseif health < 1000 and health >= 600 then
						status_string = status_string.."Корабль выглядит слегка поврежденным.";
					elseif health < 600 and health >= 250 then
						status_string = status_string.."Судя по всему, корабль получил умеренные повреждения, но на нем все еще безопасно плавать.";
					else
						status_string = status_string.."Корабль выглядит сильно поврежденным. Вероятно, плыть на нем будет очень опасно.";
					end
				else
					if health >= 500 then
						status_string = status_string.."Корабль в прекрасном состоянии.";
			...
			if entity.destination then
				if cwSailing.shipDestinations[entity.destination] then
					status_string = status_string.." Ты помнишь, что этот "..entity.longshipType.." отправляется в  "..tostring(cwSailing.shipDestinations[entity.destination].name)..".";
				end
			end
	...
	if (trace.Entity) then
		...
		if (entity:GetClass() == "cw_longship") then
			...
			if repairItemTable then
				...
				Schema:EasyText(player, "chocolate", "Без дерева ты не можешь починить корабль!");
			end
		end
	end;
end);
		...
		if (entity:GetClass() == "cw_longship" or entity:GetClass() == "cw_longship_ironclad") then
			if !entity.destination then
				if !entity.ignited then
					...
					Schema:EasyText(player, "maroon", "Ты собираешься плыть на ГОРЯЩЕМ КОРАБЛЕ?!");
				end
			end
		end
	end;
	...
	if (trace.Entity) then
		...
		if (entity:GetClass() == "cw_longship" or entity:GetClass() == "cw_longship_ironclad") then
			if entity.enchantment then
				if cwSailing.hellSailingEnabled then
					if !entity.destination then
						if !entity.ignited then
							...
							Schema:EasyText(player, "maroon", "Ты серьезно хочешь отправиться в путь на ГОРЯЩЕМ КОРАБЛЕ?!");
						end
					end
				else
					Schema:EasyText(player, "chocolate", "Одна лишь мысль о плавании в Ад вбивает гвоздь в ваш разум. Боги обязаны об этом знать.");
				...
				Schema:EasyText(player, "chocolate", "Вашей ладье не хватает чар, необходимых для безопасного плавания по реке Стикс.");
			end
		end
	end;
end);
	...
	if (trace.Entity) then
		...
		if (entity:GetClass() == "cw_gorewatchalarm") and player:GetPos():Distance2D(entity:GetPos()) < 300 then
			if !entity:GetNWBool("broken") then
				Schema:EasyText(player, "chocolate", "Вы починили систему оповещения!");
			...
			if repairItemTable then
				Clockwork.player:SetAction(player, "repair_alarm", 30, 1, function() 
					if entity:IsValid() and entity:GetNWBool("broken") and player:HasItemInstance(repairItemTable) then
						...
						Clockwork.chatBox:AddInRadius(nil, "localevent", "С лязгом шестерёнок отремонтированная система сигнализации снова активируется.", entity:GetPos(), 666);
						
						player:TakeItem(repairItemTable, true);
					end
				end);
			else
				Schema:EasyText(player, "chocolate", "Тебе нечем чинить!");
			end
		end
	end;
end);
...
concommand.Add("cw_SteamEngineRepair", function(player, cmd, args)
	...
	if (trace.Entity) then
		...
		if (entity:GetClass() == "cw_steam_engine") and player:GetPos():Distance2D(entity:GetPos()) < 300 then
			if !entity:GetNWBool("broken") then
				Schema:EasyText(player, "chocolate", "Паровой двигатель работает!");
						...
						Clockwork.player:SetAction(player, "repair_steam_engine", 30, 1, function() 
							if entity:IsValid() and entity:GetNWBool("broken") then
								...
								if #scrapItems == 3 then
									...
									Clockwork.chatBox:AddInRadius(nil, "localevent", "Под лязг шестерёнок отремонтированный паровой двигатель снова заработал..", entity:GetPos(), 666);

									return;
								end
								
								Schema:EasyText(player, "chocolate", "У вас недостаточно лома для ремонта паровой машины! Вам требуется "..tostring(scrapRequired).." скрапа!");
							end
						end);
			...
			Schema:EasyText(player, "chocolate", "Тебе нужно шесть скрапа для починки двигателя броненосца!");
		end
	end;
end);
		...
		if (entity:GetClass() == "cw_steam_engine") and player:GetPos():Distance2D(entity:GetPos()) < 300 then
			if args[1] == "refuel" then
				...
				if refuelItemTable then
					...
					Schema:EasyText(player, "chocolate", "У тебя нету угля!!");
				end
			else
				local fuel = entity.fuel;
				
				if fuel and fuel > 0 then
					Schema:EasyText(player, "olivedrab", "Бак заполнен углем на "..tostring(math.Round(fuel, 2)).."%.");
				else
					Schema:EasyText(player, "chocolate", "Запасы угля истощились!!");
				end
			end
		end
	end;
			...
			if args[1] == "on" then
				...
				if !bSuccess then
					if fault then
						...
						Schema:EasyText(player, "chocolate", "Двигатель Запущен!");
					end
				end
			else
				...
				if !bSuccess then
					if fault then
						...
						Schema:EasyText(player, "chocolate", "Двигатель Отключен!");
					end
				end
			end
		end

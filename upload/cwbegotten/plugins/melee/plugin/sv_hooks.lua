		...
		if entity:IsPlayer() then
			...
			if IsValid(entWeapon) and (entWeapon.Base == "begotten_firearm_base" or entWeapon.isJavelin) and !entity:GetNWBool("Guardening") then
				if !(entWeapon.isJavelin and cwBeliefs and entity:GetNWBool("ThrustStance") and entity:HasBelief("strength")) then
					local dropMessages = {" вылетает из его рук!", " падает из его рук прямо на землю!"};
...
function cwMelee:PlayerStabilityFallover(player, falloverTime, bNoBoogie, bNoText)
	...
	if !bNoText then
		local randomPhrases = {
			"сбит с ног!",
			"теряет равновесие и валится наземь!",
			"с грохотом падает на землю!",
			"с ужасным грохотом валится наземь!",
		}

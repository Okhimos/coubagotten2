								...
								if !player:GetNetVar("ThrustStance") then
									if (activeWeapon.isJavelin) then
										...
										if IsValid(player.possessor) then
											player.possessor:PrintMessage(HUD_PRINTTALK, "*** Перешел в стойку ближнего боя.")
										end
									elseif (activeWeapon.CanSwipeAttack == true) then
										player:PrintMessage(HUD_PRINTTALK, "*** Перешел в положение для взмаха.")
										
										if IsValid(player.possessor) then
											player.possessor:PrintMessage(HUD_PRINTTALK, "*** Перешел в положение для взмаха.")
										end
									elseif canThrust then
										player:PrintMessage(HUD_PRINTTALK, "*** Перешел в положение для тычков.")
										
										if IsValid(player.possessor) then
											player.possessor:PrintMessage(HUD_PRINTTALK, "*** Перешел в положение для тычков.")
										end
									end;
									...
									if (activeWeapon.isJavelin) then
										player:PrintMessage(HUD_PRINTTALK, "*** Перешел в положение для броска.")
										
										if IsValid(player.possessor) then
											player.possessor:PrintMessage(HUD_PRINTTALK, "*** Перешел в положение для броска.")
										end
									elseif (activeWeapon.CanSwipeAttack == true) and canThrust then
										player:PrintMessage(HUD_PRINTTALK, "*** Перешел в положение для тычка.")
										
										if IsValid(player.possessor) then
											player.possessor:PrintMessage(HUD_PRINTTALK, "*** Перешел в положение для тычка.")
										end
									elseif (attackTable["dmgtype"] == 128) then
										player:PrintMessage(HUD_PRINTTALK, "*** Перешел в положение для дробящего удара.")
										
										if IsValid(player.possessor) then
											player.possessor:PrintMessage(HUD_PRINTTALK, "*** Перешел в положение для дробящего удара.")
										end
									else
										player:PrintMessage(HUD_PRINTTALK, "*** Перешел в положение для режущего удара.")
										
										if IsValid(player.possessor) then
											player.possessor:PrintMessage(HUD_PRINTTALK, "*** Перешел в положение для режущего удара.")
										end
									end;
